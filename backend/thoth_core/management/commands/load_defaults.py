...
'''
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--only-level',
            type=int,
            help='Run only a specific level (1-5)'
        )
        parser.add_argument(
            '--no-cache-clear',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if any([clean_db_structure, clean_workspaces, clean_ai_config, clean_vector_dbs, clean_users]):
        ...
        levels = {
            1: {
                ...
                'commands': ['import_groups', 'import_users', 'import_basicaimodels', 'import_vectordb']
            },
            2: {
                'name': 'AI Model Dependencies',
                'commands': ['import_aimodels']
            },
            3: {
        ...
        for level_num in sorted(levels.keys()):
            ...
            if skip_levels and level_num in skip_levels:
                self.stdout.write(self.style.WARNING(f'Skipping Level {level_num}: {levels[level_num]["name"]}'))
                continue
            ...
            for command in level_info['commands']:
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f'✓ {command} completed in {command_duration.total_seconds():.2f}s')
                    )
    ...
    def clean_existing_data(self, clean_db_structure=False, clean_workspaces=False, 
                           clean_ai_config=False, clean_vector_dbs=False, clean_users=False):
        ...
        if clean_users:
            self.stdout.write(self.style.WARNING('  WARNING: Deleting all users including superusers!'))
        ...
        if total_deleted > 0:
            self.stdout.write(self.style.SUCCESS(f'  Data cleanup completed successfully. Total records deleted: {total_deleted}'))
        else:
    ...
    def verify_imports(self):
        ...
        try:
            ...
            from thoth_core.models import (
                ...
                Workspace, SqlColumn, Relationship
            )
            ...
            for model, name in models_to_check:
                try:
                    ...
                    self.stdout.write(self.style.ERROR(f'✗ {name}: Error counting - {str(e)}'))
