...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        csv_path = os.path.join(settings.BASE_DIR, 'setup_csv', 'aimodel.csv')
        source_specific_path = os.path.join(settings.BASE_DIR, 'setup_csv', source, 'aimodel.csv')
        ...
        if not os.path.exists(csv_path):
            self.stdout.write(self.style.ERROR(f'AI Model CSV file not found at {csv_path}'))
            return
        ...
        model_fields = [field.name for field in AiModel._meta.get_fields() 
        ...
        with open(csv_path, 'r', encoding='utf-8') as file:
            ...
            for row in reader:
                try:
                    ...
                    model_id = row.get('id')
                    if not model_id:
                        self.stdout.write(self.style.WARNING(f"Missing ID for AiModel {row.get('name', 'unknown')}, skipping"))
                        continue
                    ...
                    try:
                        ...
                        obj.save()
                        updated_count += 1
                        self.stdout.write(f"Updated AiModel with ID {model_id}: {obj.name}")
                    except AiModel.DoesNotExist:
                        # Crea un nuovo oggetto con l'ID specificato
                        valid_fields['id'] = model_id
                        obj = AiModel.objects.create(**valid_fields)
                        imported_count += 1
                        self.stdout.write(f"Created AiModel with ID {model_id}: {obj.name}")
                
                except Exception as e:
                    error_count += 1
                    self.stdout.write(
                        self.style.ERROR(f"Error processing AiModel {row.get('name', 'unknown')}: {str(e)}")
                    )
                    logger.error(f"Error processing AiModel {row.get('name', 'unknown')}: {str(e)}", exc_info=True)
    ...
    def get_defaults_from_row(self, row):
        ...
        defaults = {
            ...
            'name': row.get('name', ''),
            'url': row.get('url', ''),
            'temperature_allowed': row.get('temperature_allowed', 'True').lower() == 'true',
        }
