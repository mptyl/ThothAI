...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        source_specific_path = os.path.join(settings.BASE_DIR, 'setup_csv', source, 'setting.csv')
        ...
        if not os.path.exists(csv_path):
            self.stdout.write(self.style.ERROR(f'Setting CSV file not found at {csv_path}'))
            return
                ...
                try:
                    ...
                    if not model_id:
                        ...
                        obj, created = Setting.objects.update_or_create(
                            name=row['name'],
                            defaults=self.get_defaults_from_row(row)
                        )
                        if created:
                            imported_count += 1
                        ...
                        try:
                            ...
                            obj.save()
                            updated_count += 1
                            self.stdout.write(f"Updated Setting with ID {model_id}: {obj.name}")
                            ...
                            obj.save()
                            imported_count += 1
                            self.stdout.write(f"Created Setting with ID {model_id}: {obj.name}")
                    
                except Exception as e:
                    error_count += 1
                    self.stdout.write(
                        self.style.ERROR(f"Error processing Setting {row.get('name', 'unknown')}: {str(e)}")
                    )
    ...
    def get_defaults_from_row(self, row):
        ...
        if 'example_rows_for_comment' in row and row['example_rows_for_comment']:
            try:
                defaults['example_rows_for_comment'] = int(row['example_rows_for_comment'])
        ...
        if 'use_value_description' in row:
            defaults['use_value_description'] = self.parse_boolean(row['use_value_description'])
