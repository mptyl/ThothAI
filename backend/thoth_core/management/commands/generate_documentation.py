...
from thoth_core.thoth_ai.thoth_workflow.generate_db_documentation import generate_db_documentation as generate_documentation_action


class Command(BaseCommand):
    help = 'Generate documentation for specified databases using AI'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--database',
            type=str,
            help='Database name to generate documentation for'
        )
        parser.add_argument(
            '--all',
            action='store_true',
            help='Generate documentation for all databases'
        )
    ...
    def handle(self, *args, **options):
        ...
        if generate_all:
            ...
            self.stdout.write(f'Generating documentation for all {databases.count()} databases...')
        elif database_name:
            try:
            ...
            self.stdout.write(f'Generating documentation for workspace database: {workspace.sql_db.name}')
        ...
        try:
            with transaction.atomic():
                generate_documentation_action(mock_modeladmin, mock_request, databases)
                self.stdout.write(self.style.SUCCESS(
                    f'Successfully generated documentation for {databases.count()} database(s)'
                ))
        except Exception as e:
            self.stdout.write(self.style.ERROR(f'Error generating documentation: {str(e)}'))
