...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not os.path.exists(csv_path):
            self.stdout.write(
                self.style.ERROR(f"SqlDb CSV file not found at {csv_path}")
            )
            return
        ...
        with open(csv_path, "r", encoding="utf-8") as file:
            ...
            for row in reader:
                try:
                    ...
                    # Get the associated VectorDb if present
                    vector_db = None
                    if row.get("vector_db"):
                        try:
                            ...
                            try:
                                # Try to get by name
                                vector_db = VectorDb.objects.filter(
                                    name=row["vector_db"]
                                ).first()
                            except Exception:
                                self.stdout.write(
                    ...
                    if not model_id:
                        ...
                        if row.get("db_port") and row.get("db_port").strip():
                            try:
                                ...
                                self.stdout.write(
                                    self.style.WARNING(
                                        f"Invalid db_port value '{row.get('db_port')}' for SqlDb '{row.get('name')}', setting to None"
                                    )
                                )
                                db_port = None
                        ...
                        try:
                            ...
                            updated_count += 1
                            self.stdout.write(
                                f"Updated SqlDb with ID {model_id}: {obj.name}"
                            )
                            ...
                            imported_count += 1
                            self.stdout.write(
                                f"Created SqlDb with ID {model_id}: {obj.name}"
                            )
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f"Error processing SqlDb {row.get('name', 'unknown')}: {str(e)}"
                        )
                    )
