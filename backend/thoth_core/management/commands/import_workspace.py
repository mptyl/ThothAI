...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not os.path.exists(csv_path):
            self.stdout.write(
                self.style.ERROR(f"Workspace CSV file not found at {csv_path}")
            )
            return
            ...
            for row in reader:
                try:
                    ...
                    if not agents.get("kw_sel_agent"):
                        ...
                        if chosen_legacy:
                            try:
                                agents["kw_sel_agent"] = Agent.objects.get(
                                    id=chosen_legacy
                                )
                                self.stdout.write(
                                    "Mapped legacy kw_sel_agent_1/2 to kw_sel_agent"
                                )
                            except (Agent.DoesNotExist, ValueError):
                                pass
                    ...
                    if row.get("default_model"):
                        ...
                        try:
                            ...
                            if agent.ai_model:
                                ...
                                self.stdout.write(
                                    f"Migrated default_agent '{agent.name}' to default_model '{default_model.name}' for Workspace '{row.get('name', 'unknown')}'"
                                )
                        except (Agent.DoesNotExist, ValueError):
                            self.stdout.write(
                                self.style.WARNING(
                    ...
                    if not model_id:
                        self.stdout.write(
                            self.style.WARNING(
                                f"Missing ID for Workspace {row.get('name', 'unknown')}, skipping"
                            )
                        )
                        continue
                    ...
                    try:
                        ...
                        updated_count += 1
                        self.stdout.write(
                            f"Updated Workspace with ID {model_id}: {obj.name}"
                        )
                        ...
                        if "users" in row and row["users"]:
                            try:
                                user_ids = [
                                    int(uid.strip())
                                    for uid in row["users"].split(",")
                                    if uid.strip()
                                ]
                                existing_users = User.objects.filter(id__in=user_ids)
                                obj.users.set(existing_users)
                                self.stdout.write(
                                    f"Set {len(existing_users)} users for workspace {obj.name}"
                                )
                        ...
                        if "default_workspace" in row and row["default_workspace"]:
                            try:
                                default_user_ids = [
                                    int(uid.strip())
                                    for uid in row["default_workspace"].split(",")
                                    if uid.strip()
                                ]
                                existing_default_users = User.objects.filter(
                                    id__in=default_user_ids
                                )
                                obj.default_workspace.set(existing_default_users)
                                self.stdout.write(
                                    f"Set {len(existing_default_users)} default users for workspace {obj.name}"
                                )
                            except Exception as e:
                                self.stdout.write(
                                    self.style.WARNING(
                        ...
                        imported_count += 1
                        self.stdout.write(
                            f"Created Workspace with ID {model_id}: {obj.name}"
                        )
