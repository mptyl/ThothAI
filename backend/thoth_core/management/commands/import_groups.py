...
class Command(BaseCommand):
    help = (
        "Import Django auth groups from a CSV file and their associated group profiles"
    )
                ...
                try:
                    ...
                    if permissions_field:
                        try:
                            ...
                            permission_ids = [
                                int(pid.strip())
                                for pid in permissions_field.split(",")
                                if pid.strip()
                            ]
                            ...
                            self.stdout.write(
                                f"Set {permissions.count()} permissions for group: {group.name}"
                            )
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f"Error processing group {row.get('name', 'unknown')}: {str(e)}"
                        )
                    )
                ...
                for row in reader:
                    try:
                        ...
                        if not group_id:
                            self.stdout.write(
                                self.style.WARNING(
                                    f"Missing group ID in row {reader.line_num}"
                                )
                            )
                            profile_errors += 1
                            continue
                        ...
                        explain_generated_query = self.parse_boolean(
                            row.get("explain_generated_query", "True")
                        )
                        ...
                        if model_id:
                            ...
                            try:
                                ...
                                group_profile.explain_generated_query = (
                                    explain_generated_query
                                )
                                group_profile.save()
                                profiles_updated += 1
                                self.stdout.write(
                                    f"Updated GroupProfile for: {group.name}"
                                )
                                ...
                                profiles_imported += 1
                                self.stdout.write(
                                    f"Created GroupProfile for: {group.name}"
                                )
                        else:
                            # If no ID, use update_or_create with group
                            group_profile, created = (
                        ...
                        self.stdout.write(
                            self.style.ERROR(
                                f"Error processing GroupProfile for group ID {group_id}: {str(e)}"
                            )
                        )
        else:
            self.stdout.write(
                self.style.WARNING(
                    f"GroupProfile CSV file not found at {groupprofile_path}"
                )
            )
