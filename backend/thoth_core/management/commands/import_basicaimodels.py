...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        csv_path = get_setup_csv_path("basicaimodel.csv", source)
        
        self.stdout.write(f"Using CSV file at: {csv_path}")

        if not os.path.exists(csv_path):
            self.stdout.write(
                self.style.ERROR(f"BasicAiModel CSV file not found at {csv_path}")
            )
            return
        ...
        if not csv_data:
            self.stdout.write(
                self.style.WARNING("CSV file is empty. No records to import.")
            )
            return
        ...
        with transaction.atomic():
            ...
            self.stdout.write(
                self.style.WARNING(
                    f"Deleted {deleted_count} existing BasicAiModel records"
                )
            )
            ...
            for row in csv_data:
                try:
                    ...
                    imported_count += 1
                    self.stdout.write(
                        f"Imported BasicAiModel: {model.name} (ID: {model.id})"
                    )
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f"Error importing BasicAiModel {row.get('name', 'unknown')}: {str(e)}"
                        )
                    )
