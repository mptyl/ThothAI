        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            ...
            try:
                ...
                raise LLMTimeoutError(
                    f"LLM operation timed out after {seconds} seconds"
                )
...
def create_table_comments(modeladmin, request, queryset):  # Renamed function
    ...
    try:
        ...
        if total_tables > chunk_size:
            modeladmin.message_user(
                request,
                f"Processing {total_tables} tables in chunks of {chunk_size}.",
                level=messages.INFO,
            )
        ...
        modeladmin.message_user(
            request,
            f"Finished processing all {total_tables} tables.",
            level=messages.SUCCESS,
        )

    except Exception as e:
        modeladmin.message_user(
            request, f"Failed to create table comments: {str(e)}", level=messages.ERROR
        )
        return
...
def process_table_chunk(modeladmin, request, chunk, chunk_number, setting):
    for table in chunk:
        ...
        try:
            ...
            modeladmin.message_user(
                request,
                f"Error setting up SQL database for table {table.name}: {str(e)}. Skipping this table.",
                level=messages.ERROR,
            )
        ...
        table_current_comment_val = (
            table.description
            if table.description and table.description.strip()
            else table.generated_comment
        )
        table_current_comment_str = (
            table_current_comment_val
            if table_current_comment_val and table_current_comment_val.strip()
            else ""
        )

        # Get column comments
        column_comments_df = create_column_comments_dataframe(
            table
        )  # Returns a DataFrame
        ...
        try:
            ...
            if examples_data_result and any(
                ...
                if examples_data_result is None:
                    modeladmin.message_user(
                        request,
                        f"Info for table {table.name}: No example data returned (None). LLM will receive empty string.",
                        level=messages.INFO,
                    )
                else:
                    modeladmin.message_user(
                        request,
                        f"Info for table {table.name}: No example data found (empty). LLM will receive empty string.",
                        level=messages.INFO,
                    )
        except Exception as e:
            ...
            modeladmin.message_user(
                request,
                f"Warning for table {table.name}: Could not retrieve example data: {str(e)}. LLM will receive empty string.",
        ...
        try:
            ...
            if table_comment_json is None:
                ...
                modeladmin.message_user(
                    request,
                    f"Error for table {table.name}: No JSON-like content found in the LLM response. Raw output preview: {raw_output_preview}...",
                    level=messages.ERROR,
                )
                continue  # Skip to the next table
            else:
                # Save the table comment in JSON format
                return_code = update_table_comments_on_backend(
                    table_comment_json, table
                )
                if return_code == "OK":
                    modeladmin.message_user(
                        request, f"Comment created successfully for table {table.name}."
                    )
                else:
                    modeladmin.message_user(
                        request,
                        f"Error updating comment for table {table.name}: {return_code}",
                        level=messages.ERROR,
                    )
        except Exception as e:
            modeladmin.message_user(
                request,
                f"Error during pipeline execution or comment update for table {table.name}: {str(e)}",
                level=messages.ERROR,
            )
            continue  # Skip to the next table
...
def update_table_comments_on_backend(table_comment_json, table):
    ...
    if (
        table_comment_json
        and isinstance(table_comment_json, list)
        and len(table_comment_json) > 0
    ):
...
def create_table_comments_async(
    table_ids: List[int], workspace_id: int = None, custom_logger: logging.Logger = None
) -> Dict[str, Any]:
    """
    ...
    try:
        if custom_logger:
            logger.info(
                f"Starting table comment generation for {len(table_ids)} tables"
            )
        else:
            logger.info(
                f"Starting async table comments generation for {len(table_ids)} tables"
            )
        ...
        for table in tables:
            ...
            try:
                with transaction.atomic():
                    if custom_logger:
                        logger.info(
                            f"Processing table {table_count}/{total_tables}: {table.name}"
                        )
                    else:
                        logger.info(
                            f"Starting processing table: {table.name} (ID: {table.id})"
                        )
                    ...
                    try:
                        ...
                        if custom_logger:
                            logger.info(
                                f"Setting up database connection for {table.name}"
                            )
                        else:
                            logger.info(
                    ...
                    column_comments_str = (
                        column_comments_df.to_markdown(index=False)
                        if not column_comments_df.empty
                        else ""
                    )
                    ...
                    try:
                        ...
                        if custom_logger:
                            logger.warning(
                                f"Could not retrieve example data for {table.name}"
                            )
                        else:
                            logger.warning(
                                f"Could not retrieve example data for {table.name}: {str(e)}"
                            )
                        example_data_str = ""
                    ...
                    def run_llm_generation():
                        with (
                            llm_timeout(30) as timeout_wrapper
                        ):  # 30 seconds timeout - if LLM doesn't respond, something is wrong
                            prompt_variables = {
                                "database_schema": table_schema,
                                "table_comment": table_current_comment_str,
                                "column_comments": column_comments_str,
                    ...
                    try:
                        ...
                        if custom_logger:
                            logger.info(
                                f"LLM response received for table: {table.name}"
                            )
                        ...
                        if table_comment_json:
                            ...
                            if return_code == "OK":
                                if custom_logger:
                                    logger.info(
                                        f"✓ Successfully processed table: {table.name}"
                                    )
                                else:
                                    logger.info(
                                        f"Successfully processed table {table.name}"
        ...
        logger.info(
            f"Completed processing all tables. Processed: {results['processed']}, Failed: {results['failed']}"
        )
        return results
