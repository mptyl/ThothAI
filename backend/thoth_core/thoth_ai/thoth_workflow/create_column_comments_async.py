...
from thoth_core.models import Setting, LLMChoices, SqlColumn, SqlTable, Workspace
    ...
    try:
        ...
        try:
            ...
            if table_comment_json is None:
                ...
                logger.error(f"No JSON found in LLM response. Raw output: {str(output)[:500]}")
                return f"Error: No JSON-like content found in the response"
            else:
                logger.info(f"Parsed JSON successfully: {len(table_comment_json)} items")
    ...
    if columns_comment_json and isinstance(columns_comment_json, list):
        try:
            ...
            for i, item in enumerate(columns_comment_json):
                try:
                    if isinstance(item, dict):
                        ...
                        for desc_key in ["description", "comment", "desc", "generated_comment"]:
                            if desc_key in item:
                        ...
                        if column_name and description:
                            ...
                            logger.warning(f"Skipping item {i} with missing data. column_name={column_name}, description={description}")
                            logger.warning(f"Item keys: {list(item.keys())}, Item: {item}")
                    else:
            ...
            if columns_comment_json:
                logger.error(f"First item type: {type(columns_comment_json[0]) if columns_comment_json else 'empty'}")
            import traceback
            logger.error(f"Traceback: {traceback.format_exc()}")
            return f"Error processing JSON: {str(e)}"
    else:
        logger.error(f"Invalid JSON format. Type: {type(columns_comment_json)}, Content: {columns_comment_json}")
