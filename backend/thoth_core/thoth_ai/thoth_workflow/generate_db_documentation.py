...
from thoth_core.models import SqlDb, SqlTable, SqlColumn, Relationship, Setting, LLMChoices
from thoth_core.thoth_ai.thoth_workflow.comment_generation_utils import setup_default_comment_llm_model
from thoth_core.utilities.utils import get_exports_directory, ensure_exports_directory, get_docker_friendly_error_message
...
def generate_scope_html(scope_json_str):
    ...
    try:
        ...
        for key, value in scope_data.items():
            ...
            friendly_title = re.sub(r'([a-z])([A-Z])', r'\1 \2', key)  # Add space before capitals
            friendly_title = friendly_title.replace('_', ' ')  # Replace underscores with spaces
...
def generate_relationships_html(db_id):
    ...
    relationships = Relationship.objects.filter(
        source_table__sql_db__id=db_id
    ...
    if relationships.exists():
        ...
        html_parts.append('<p>No foreign key relationships defined in this database.</p>')
        ...
        for column in columns:
            ...
            if column.fk_field:
                if isinstance(column.fk_field, str) and '.' in column.fk_field:
                    ...
                    fk_defs.append(f"FOREIGN KEY ({column.original_column_name}) REFERENCES {ref_table}({ref_column})")
...
def generate_simple_erd_svg(tables_data, relationships_data):
    ...
    for i, table in enumerate(tables_data):
        ...
        y = margin + row * (table_height + spacing + row_height * min(5, len(table['columns'])))
        table_positions[table['name']] = {'x': x, 'y': y, 'width': table_width, 'height': table_height + row_height * min(5, len(table['columns']))}
    ...
    # Generate SVG
    svg_parts = [f'<svg width="{max_x}" height="{max_y}" xmlns="http://www.w3.org/2000/svg">']
    ...
    svg_parts.append('.relationship { stroke: #666; stroke-width: 2; fill: none; marker-end: url(#arrowhead); }')
    svg_parts.append('</style>')
    
    # Add arrowhead marker
    svg_parts.append('<defs>')
    svg_parts.append('<marker id="arrowhead" markerWidth="10" markerHeight="10" refX="9" refY="3" orient="auto">')
    ...
    for table in tables_data:
        ...
        # Header background
        svg_parts.append(f'<rect class="table-header" x="0" y="0" width="{table_width}" height="{table_height}" />')
        
        # Table body background
        body_height = row_height * min(5, len(table['columns']))
        svg_parts.append(f'<rect class="table" x="0" y="{table_height}" width="{table_width}" height="{body_height}" />')
        
        # Table name
        svg_parts.append(f'<text class="table-text header-text" x="{table_width/2}" y="20" text-anchor="middle">{table["name"]}</text>')
        ...
        for i, col in enumerate(table['columns'][:5]):
            ...
            if col['is_pk']:
                ...
                col_text = f"🔗 {col_text}"
            svg_parts.append(f'<text class="column-text" x="10" y="{y_pos}">{col_text}</text>')
        
        if len(table['columns']) > 5:
            y_pos = table_height + 20 + 5 * row_height
            svg_parts.append(f'<text class="column-text" x="10" y="{y_pos}">... +{len(table["columns"]) - 5} more</text>')
        
        svg_parts.append('</g>')
    
    # Draw relationships
    for rel in relationships_data:
        if rel['source_table'] in table_positions and rel['target_table'] in table_positions:
            ...
            mid_x = (x1 + x2) / 2
            svg_parts.append(f'<path class="relationship" d="M {x1} {y1} Q {mid_x} {y1} {mid_x} {(y1+y2)/2} Q {mid_x} {y2} {x2} {y2}" />')
...
def generate_db_documentation(modeladmin, request, queryset):
    ...
    try:
        ...
        if not hasattr(request, 'current_workspace') or not request.current_workspace:
            modeladmin.message_user(request, "No active workspace found. Please select a workspace.", messages.ERROR)
            return
        ...
        if queryset.count() > 1:
            modeladmin.message_user(request, f"Multiple databases selected. Processing only the first one.", messages.WARNING)
        ...
        for table in SqlTable.objects.filter(sql_db=db):
            ...
            for col in SqlColumn.objects.filter(sql_table=table):
                columns_data.append({
                    ...
                    'description': col.column_description or col.generated_comment or "",
        ...
        try:
            ...
            if error_message:
                modeladmin.message_user(request, f"Export failed: {error_message}", messages.ERROR)
                return
            ...
            try:
                ...
                modeladmin.message_user(request, f"Failed to create database directory: {docker_error}", messages.ERROR)
                return
            ...
            try:
                ...
                # Success message
                success_msg = f"Successfully generated documentation for database '{db.name}':\n"
                success_msg += f"- HTML: {io_dir}/{db.name}/{html_filename}\n"
                success_msg += f"- ERD Mermaid diagram saved to database field"
                
                modeladmin.message_user(request, success_msg, level=messages.SUCCESS)
                    
            except (OSError, PermissionError) as e:
                docker_error = get_docker_friendly_error_message(e)
                modeladmin.message_user(request, f"Failed to save documentation: {docker_error}", level=messages.ERROR)
                
        except Exception as e:
            modeladmin.message_user(request, f"Error generating documentation for database '{db.name}': {str(e)}", level=messages.ERROR)
    
    except FileNotFoundError:
        modeladmin.message_user(request, "Prompt template file not found.", level=messages.ERROR)
    except Exception as e:
        modeladmin.message_user(request, f"An unexpected error occurred: {str(e)}", level=messages.ERROR)

generate_db_documentation.short_description = "Generate database documentation (AI assisted)"