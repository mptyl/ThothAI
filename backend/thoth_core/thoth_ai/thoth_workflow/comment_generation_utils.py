    ...
    try:
        ...
        if plugin_db_type == "sqlite":
            ...
            sqlite_dir = os.path.join(
                db_root_path, f"{sql_db.db_mode}_databases", sql_db.db_name
            )
            connection_params["database_path"] = os.path.join(
                sqlite_dir, f"{sql_db.db_name}.sqlite"
            )
        else:
...
def get_table_schema_safe(db_manager, table_name):
    ...
    try:
        ...
        if hasattr(db_manager, "get_table_schema") and callable(
            getattr(db_manager, "get_table_schema")
        ):
