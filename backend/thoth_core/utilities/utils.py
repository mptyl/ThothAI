...
def export_selected_tables_to_csv(modeladmin, request, queryset):
    ...
    for table in queryset:
        writer.writerow(
            [table.sql_db.id, table.name, table.description, table.generated_comment]
        )
...
def import_csv(modeladmin, request, queryset):
    ...
    if error_message:
        modeladmin.message_user(
            request, f"Import failed: {error_message}", level="error"
        )
        return
    ...
    with default_storage.open(file_path, "r") as file:
        ...
        for row in reader:
            try:
                with transaction.atomic():
                    ...
                    if pk_value:
                        ...
                        instance = Model.objects.create(
                            **{k: v for k, v in instance_data.items() if k != "id" or v}
                        )
                    ...
                    if model_name == "workspace":
                        ...
                        instance.preprocessing_status = (
                            Workspace.PreprocessingStatus.IDLE
                        )
                        ...
                        instance.column_comment_status = (
                            Workspace.PreprocessingStatus.IDLE
                        )
                ...
                error_count += 1
                modeladmin.message_user(
                    request, f"Error on row {reader.line_num}: {str(e)}", level="error"
                )
...
def export_db_structure_to_csv(modeladmin, request, queryset):
    ...
    for db in queryset:
        ...
        with open(tables_file, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["id", "name", "description", "generated_comment", "sql_db"]
            )
            for table in tables:
                writer.writerow(
                    [
...
def initialize_database_plugins():
    ...
    try:
        ...
        available_list = [
            db for db, available in available_databases.items() if available
        ]
        unavailable_list = [
            db for db, available in available_databases.items() if not available
        ]
