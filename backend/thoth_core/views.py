...
from .serializers import SqlColumnSerializer, SqlTableSerializer, UserSerializer, WorkspaceSerializer, WorkspaceListSerializer, ThothLogSerializer, AgentSerializer
...
def test_api_key(request):
    ...
    return Response({"message": "API key authentication successful!", "user": str(request.user), "auth": str(request.auth)})
...
def get_user_workspaces_list(request):
    ...
    # Debug logging
    import sys
    print(f"get_user_workspaces_list called", file=sys.stderr)
    print(f"Request user: {request.user}", file=sys.stderr)
    print(f"Request auth: {request.auth}", file=sys.stderr)
    print(f"Request headers X-API-KEY: {request.headers.get('X-API-KEY')}", file=sys.stderr)
...
def get_workspace_by_name(request, workspace_name):
    ...
    if request.auth is True and request.user is None:
        ...
        workspaces = Workspace.objects.filter(name=workspace_name).select_related(
            'sql_db__vector_db',
            'default_model__basic_model',
            'question_validator__ai_model__basic_model',
            'kw_sel_agent__ai_model__basic_model',
        ...
        user = request.user
        logger.info(f"Getting workspace '{workspace_name}' for authenticated user {user.username}")
    ...
    if workspace_count == 0:
        ...
        logger.warning(f"Multiple workspaces ({workspace_count}) found with name '{workspace_name}'")
        return Response(
...
def get_workspace_by_id(request, workspace_id):
    ...
    if request.auth is True and request.user is None:
        # API key authentication - search all workspaces
        logger.info(f"Getting workspace with ID {workspace_id} for API key authentication")
        try:
            workspace = Workspace.objects.select_related(
                ...
                'ask_human_help_agent__ai_model__basic_model',
                'setting__comment_model__basic_model'
            ).prefetch_related('users', 'default_workspace').get(pk=workspace_id)
        except Workspace.DoesNotExist:
        ...
        # Token or session authentication - search user's workspaces only
        user = request.user
        logger.info(f"Getting workspace with ID {workspace_id} for authenticated user {user.username}")
        try:
            workspace = Workspace.objects.filter(users=user).select_related(
                'sql_db__vector_db',
                'default_model__basic_model',
                'question_validator__ai_model__basic_model',
                'kw_sel_agent__ai_model__basic_model',
            ...
            logger.info(f"No workspace found with ID {workspace_id} for user {user.username}")
            return Response(
                {"error": f"No workspace found with ID {workspace_id}"}, 
                status=status.HTTP_404_NOT_FOUND
            )
...
def get_workspace_agent_pools(request, workspace_id):
    ...
    for agent in agents:
        ...
        if agent.ai_model:
            agent_data["ai_model"] = {
                ...
                "context_size": agent.ai_model.context_size if hasattr(agent.ai_model, 'context_size') else None
            }
    ...
    logger.info(f"SQL Generators - Basic: {len(result['sql_generators']['basic'])}, "
                f"Advanced: {len(result['sql_generators']['advanced'])}, "
                f"Expert: {len(result['sql_generators']['expert'])}")
    logger.info(f"Test Generators - Basic: {len(result['test_generators']['basic'])}, "
    ...
    try:
        ...
        try:
            ...
            if vector_db.vect_type.lower() == 'qdrant':
                ...
                connection_info['collection_exists'] = vector_db.name in collection_names
                ...
                end_time = time.time()
                connection_info['response_time_ms'] = round((end_time - start_time) * 1000, 2)
                
                logger.info(f"Vector DB connection test successful for workspace {workspace.name}")
...
def check_embedding_config(request, workspace_id):
    ...
    try:
        ...
        if vector_db.embedding_provider == 'openai':
            ...
            env_keys_to_check = ['HUGGINGFACE_API_KEY', 'HF_API_KEY', 'HUGGINGFACE_TOKEN']
        ...
        if len(found_env_keys) > 0:
            try:
                ...
                if vector_db.embedding_provider == 'openai':
                    try:
                        ...
                        if api_key:
                            ...
                            response = client.embeddings.create(
                                input=test_text,
                                model=vector_db.embedding_model or "text-embedding-3-small"
                            )
                    ...
                    try:
                        import cohere
                        api_key = os.environ.get('COHERE_API_KEY') or os.environ.get('COHERE_KEY')
                        if api_key:
                            co = cohere.Client(api_key)
                            response = co.embed(
                                texts=[test_text],
                                model=vector_db.embedding_model or "embed-multilingual-v3.0"
                            )
                    ...
                    try:
                        ...
                        api_key = os.environ.get('MISTRAL_API_KEY') or os.environ.get('MISTRAL_KEY')
                        if api_key:
                    ...
                    try:
                        ...
                        logger.info("Using generic vector store approach for embedding test")
                        embedding_test['test_performed'] = True
                        embedding_test['test_successful'] = True
                        embedding_test['test_error'] = "Generic test performed through vector store initialization"
                    except Exception as e:
                ...
                if embedding is not None:
                    embedding_test['test_performed'] = True
                    embedding_test['test_successful'] = True
                    embedding_test['embedding_time_ms'] = round((end_time - start_time) * 1000, 2)
                    
                    # Get embedding dimension
                    if hasattr(embedding, '__len__'):
                        embedding_test['embedding_dimension'] = len(embedding)
                    
                    logger.info(f"Embedding test successful: dimension={embedding_test['embedding_dimension']}, time={embedding_test['embedding_time_ms']}ms")
...
def get_columns_by_table(request):
    ...
    try:
        columns = SqlColumn.objects.filter(sql_table_id=table_id).order_by('original_column_name')
        column_data = [{'id': column.id, 'name': f"{column.sql_table.name}.{column.original_column_name}"} for column in columns]
...
def health_check(request):
    ...
    try:
        ...
        health_data = health_checker.perform_health_check(include_metrics=include_metrics)

        # Log health check request
        logger.info(f"Health check requested from {request.META.get('REMOTE_ADDR', 'unknown')} - Status: {health_data['status']}")
...
class ThothLogListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if hasattr(self.request, 'user') and self.request.user and not self.request.user.is_superuser:
