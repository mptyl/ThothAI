...
class RelationshipAdminForm(forms.ModelForm):
    ...
    class Meta:
        model = Relationship
        fields = ['database', 'source_table', 'target_table', 'source_column', 'target_column']
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_name in ['source_table', 'target_table', 'source_column', 'target_column']:
        ...
        if self.instance and self.instance.pk:
            ...
            if 'source_table' in self.fields:
                self.fields['source_table'].queryset = SqlTable.objects.all().order_by('sql_db__name', 'name')
            if 'target_table' in self.fields:
                self.fields['target_table'].queryset = SqlTable.objects.all().order_by('sql_db__name', 'name')
            if 'source_column' in self.fields:
                self.fields['source_column'].queryset = SqlColumn.objects.all().order_by('sql_table__sql_db__name', 'sql_table__name', 'original_column_name')
                self.fields['source_column'].label_from_instance = self.label_from_instance_column
            if 'target_column' in self.fields:
                self.fields['target_column'].queryset = SqlColumn.objects.all().order_by('sql_table__sql_db__name', 'sql_table__name', 'original_column_name')
                self.fields['target_column'].label_from_instance = self.label_from_instance_column
        else:
    ...
    def clean(self):
        ...
        if source_table and database and source_table.sql_db != database:
            raise forms.ValidationError('Source table must belong to the selected database.')
        
        # Validate that target table belongs to selected database
        if target_table and database and target_table.sql_db != database:
            raise forms.ValidationError('Target table must belong to the selected database.')
        ...
        if source_column and source_table and source_column.sql_table != source_table:
            raise forms.ValidationError('Source column must belong to the source table.')
        ...
        if target_column and target_table and target_column.sql_table != target_table:
            raise forms.ValidationError('Target column must belong to the target table.')
...
class RelationshipAdmin(admin.ModelAdmin):
    ...
    ordering = ('source_table__sql_db__name', 'source_table__name', 'target_table__name')
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == "source_table":
            ...
            kwargs["queryset"] = SqlColumn.objects.all().order_by('sql_table__sql_db__name', 'sql_table__name', 'original_column_name')
    ...
    def source_db(self, obj):
        ...
        return obj.source_table.sql_db.name if obj.source_table and obj.source_table.sql_db else "-"
