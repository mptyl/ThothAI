...
from thoth_core.models import Workspace, Agent, AiModel, SqlDb, Setting, AgentChoices, VectorDb
...
class WorkspaceAdmin(admin.ModelAdmin):
    ...
    actions = (export_csv, import_csv, 'duplicate_workspace', 'reset_comment_and_preprocessing_fields')
    readonly_fields = ('table_comment_status', 'table_comment_task_id', 'table_comment_log',
                      ...
                      'last_preprocess', 'last_evidence_load', 'last_sql_loaded', 'created_at', 'updated_at')
    
    fieldsets = (
        ('Basic Information', {
            'fields': ('name', 'level', 'description', 'users', 'default_workspace'),
        ...
        ('SQL Generation Agents', {
            'fields': ('sql_basic_agent', 'sql_advanced_agent', 'sql_expert_agent', 'number_of_sql_to_generate'),
            'classes': ('collapse',),
            'description': 'Configure agents for SQL generation at different complexity levels.'
        }),
        ('Test Generation & Execution Agents', {
            'fields': ('test_gen_agent_1', 'test_gen_agent_2', 'test_gen_agent_3', 'test_exec_agent', 'number_of_tests_to_generate', 'evaluation_threshold'),
            'classes': ('collapse',),
        ...
        ('Table Comments Status', {
            'fields': ('table_comment_status', 'table_comment_task_id', 'table_comment_log',
                      'table_comment_start_time', 'table_comment_end_time'),
            'classes': ('collapse',),
            'description': 'Status and logs for table comment generation tasks.'
        }),
        ('Column Comments Status', {
            'fields': ('column_comment_status', 'column_comment_task_id', 'column_comment_log',
                      'column_comment_start_time', 'column_comment_end_time'),
            'classes': ('collapse',),
            'description': 'Status and logs for column comment generation tasks.'
        }),
        ('Preprocessing Status', {
            'fields': ('preprocessing_status', 'task_id', 'last_preprocess_log',
                      'preprocessing_start_time', 'preprocessing_end_time',
                      'last_preprocess', 'last_evidence_load', 'last_sql_loaded'),
            'classes': ('collapse',),
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        elif db_field.name in ['question_validator', 'kw_sel_agent', 
                               'sql_basic_agent', 
                               'sql_advanced_agent', 'sql_expert_agent', 'test_gen_agent_1', 
                               'test_gen_agent_2', 'test_gen_agent_3', 'test_exec_agent', 'explain_sql_agent', 
    ...
    def duplicate_workspace(self, request, queryset):
        ...
        for workspace in queryset:
            try:
                ...
                workspace.default_workspace.set(original_workspace.default_workspace.all())
                
                duplicated_count += 1
                
            except Exception as e:
                messages.error(request, f"Error duplicating workspace '{original_name}': {str(e)}")
                continue
    ...
    def reset_comment_and_preprocessing_fields(self, request, queryset):
        ...
        for workspace in queryset:
            try:
                ...
                messages.error(request, f"Error resetting workspace '{workspace.name}': {str(e)}")
                continue
    ...
    reset_comment_and_preprocessing_fields.short_description = "Reset comment and preprocessing fields"