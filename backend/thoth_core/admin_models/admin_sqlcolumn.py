    ...
    class Meta:
        ...
        widgets = {
            ...
            "data_format": forms.Select(
                choices=ColumnDataTypes, attrs={"style": "width: 200px;"}
            ),
...
class SqlColumnAdmin(admin.ModelAdmin):
    ...
    actions = [
        ...
        "copy_original_name_to_name",
        "copy_name_to_original_name",
        "validate_fk_field_format",
        create_selected_column_comments,
        "create_column_comments_async",
    ]
    create_selected_column_comments.short_description = (
        "Generate selected columns comment (AI assisted)"
    )
    ...
    def get_db_name(self, obj):
        ...
        return (
            obj.sql_table.sql_db.name if obj.sql_table and obj.sql_table.sql_db else "-"
        )
    ...
    def copy_generated_to_description(self, request, queryset):
        ...
        if updated_count == 0:
            self.message_user(
                request,
                f"No columns updated - none of the {total_count} selected columns had text in generated_comment field.",
            )
        else:
            self.message_user(
                request,
                f"{updated_count} of {total_count} columns updated successfully (only columns with text in generated_comment were updated).",
            )

    copy_generated_to_description.short_description = (
        "Copy generated comment to column description"
    )
    ...
    copy_original_name_to_name.short_description = (
        "Copy original column name to column name"
    )
    ...
    copy_name_to_original_name.short_description = (
        "Copy column name to original column name"
    )
    ...
    def validate_fk_field_format(self, request, queryset):
        ...
        """
        error_count, success_count, error_messages, total_checked = validate_fk_fields(
            queryset, request
        )
        ...
        if error_count == 0:
            if total_checked > 0:
                messages.success(
                    request, f"All {total_checked} foreign key references are valid."
                )
            else:
    ...
    def create_column_comments_async(self, request, queryset):
        ...
        if not queryset.exists():
            self.message_user(
                request,
                "No columns selected for comment generation.",
    ...
    create_column_comments_async.short_description = (
        "Generate selected columns comment (AI assisted - async)"
    )
