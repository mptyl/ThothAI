...
class VectorDbAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'vect_type', 'embedding_provider', 'get_connection_info', 'environment')
    ...
    fieldsets = [
        (None, {
            'fields': ('name', 'vect_type')
        }),
        ('Embedding Configuration', {
            'fields': ('embedding_provider', 'embedding_model', 'embedding_base_url'),
    ...
    def duplicate_vectordb(self, request, queryset):
        ...
        for vectordb in queryset:
            try:
                ...
                messages.error(request, f"Error duplicating VectorDb '{original_name}': {str(e)}")
                continue
        
        if duplicated_count > 0:
            messages.success(request, f"Successfully duplicated {duplicated_count} VectorDb(s).")
        else:
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        if 'embedding_provider' in form.base_fields:
            form.base_fields['embedding_provider'].help_text = (
