...
from thoth_core.models import SqlDb, VectorDb, SqlTable, SqlColumn, Relationship, Workspace
from thoth_core.utilities.utils import export_csv, import_csv, export_db_structure_to_csv
from thoth_core.dbmanagement import create_tables, create_columns, create_relationships, create_db_elements
...
from thoth_core.thoth_ai.thoth_workflow.async_table_comments import start_async_column_comments, start_async_table_comments
...
class SqlDbAdminForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'user_name': forms.TextInput(attrs={'style': 'width: 500px; max-width: 100%;'}),
...
class SqlDbAdmin(admin.ModelAdmin):
    ...
    actions = (export_csv, import_csv, create_tables, create_relationships, create_db_elements, 'validate_db_fk_fields', export_db_structure_to_csv, 'duplicate_sqldb', 'associate_candidates_fk_to_pk', 'test_connection', 'generate_all_comments', generate_scope, generate_db_documentation, 'scan_gdpr_compliance')
    fieldsets = (
        ('Basic Information', {
        ...
        ('AI Configuration', {
            'fields': ('vector_db', 'language', 'scope', 'scope_json', 'directives', 'erd'),
            'classes': ('collapse',),
        }),
        ('GDPR Compliance', {
        ...
        for sqldb in queryset:
            try:
                ...
                if original_vector_db:
                    ...
                    duplicated_vector_db = VectorDb.objects.get(pk=original_vector_db.pk)
                ...
                messages.error(request, f"Error duplicating SqlDb '{original_name}': {str(e)}")
                continue
        
        if duplicated_count > 0:
            messages.success(request, f"Successfully duplicated {duplicated_count} SqlDb(s) with their associated Vector databases.")
        else:
            messages.warning(request, "No SqlDb instances were duplicated.")
    
    duplicate_sqldb.short_description = "Duplicate selected SqlDb instances with VectorDb"
    ...
    def associate_candidates_fk_to_pk(self, request, queryset):
        ...
        for sql_db in queryset:
            ...
            try:
                ...
                if not db_manager:
                    messages.error(request, f"Database '{sql_db.name}': Could not create database manager")
                    continue
                ...
                for column in all_columns:
                    ...
                    is_pk = (
                        column.pk_field and "PK" in column.pk_field.upper() or
                        column.original_column_name.lower() in ['id', 'pk'] or
                        column.original_column_name.lower().endswith('_id') and column.pk_field
                    )
                    
                    if is_pk:
                        pk_columns.append(column)
                
                messages.info(request, f"Database '{sql_db.name}': Found {len(pk_columns)} primary key columns across {SqlTable.objects.filter(sql_db=sql_db).count()} tables")
                ...
                for fk_column in all_columns:
                    ...
                    for pk_column in pk_columns:
                        ...
                        if fk_column_name == expected_camel:
                            potential_relationships.append((fk_column, pk_column, "camel_case"))
                            continue
                        ...
                        if fk_column_name == expected_concat:
                            potential_relationships.append((fk_column, pk_column, "concatenation"))
                            continue
                        
                        # Pattern 6: Table name only (tablename)
                        if fk_column_name == pk_table_name:
                            potential_relationships.append((fk_column, pk_column, "table_name_only"))
                            continue
                
                total_fks_identified += len(potential_relationships)
                messages.info(request, f"Database '{sql_db.name}': Identified {len(potential_relationships)} potential FK relationships")
                    ...
                    try:
                        ...
                        for fk_value in fk_values:
                            try:
                                ...
                                try:
                                    pk_results = db_manager.execute_sql(pk_query, (fk_value,))
                                except Exception:
                                    try:
                                ...
                                if pk_results and len(pk_results) > 0 and pk_results[0][0] > 0:
                                    valid_count += 1
                        ...
                        if validation_rate >= 0.7:
                            validated_relationships.append((fk_column, pk_column, pattern, validation_rate))
                ...
                messages.info(request, f"Database '{sql_db.name}': Validated {len(validated_relationships)} FK relationships")
                
                # Step 4: Create Relationship records
                relationships_created = 0
                
                for fk_column, pk_column, pattern, validation_rate in validated_relationships:
                    try:
                ...
                messages.info(request, f"Database '{sql_db.name}': Processed {relationships_created} relationships (some may have overridden existing records)")
                
            except Exception as e:
                messages.error(request, f"Database '{sql_db.name}': Error during FK association - {str(e)}")
                continue
    ...
    def test_connection(self, request, queryset):
        ...
        if success_count > 0 and error_count == 0:
            ...
            messages.error(
                request,
                f"All {error_count} database connection(s) failed"
            )
    ...
    def generate_all_comments(self, request, queryset):
        ...
        """
        if queryset.count() != 1:
            messages.error(request, "Please select exactly one database for comment generation.")
            return
        ...
        if not hasattr(request, 'current_workspace') or not request.current_workspace:
            messages.error(request, "No active workspace found. Please select a workspace.")
            return
        ...
        can_start_columns, column_message = check_task_can_start(workspace, 'column_comment')
        if not can_start_columns:
            messages.error(request, f"Cannot start column comment generation: {column_message}. Current column comment status: {workspace.column_comment_status}")
            return
            
        # Check if table comment generation task can be started
        can_start_tables, table_message = check_task_can_start(workspace, 'table_comment')
        if not can_start_tables:
            messages.error(request, f"Cannot start table comment generation: {table_message}. Current table comment status: {workspace.table_comment_status}")
            return
        ...
        if not all_columns.exists():
            messages.warning(request, f"No columns found in database '{sql_db.name}'. Please ensure the database structure has been imported first.")
            return
            
        if not all_tables.exists():
            messages.warning(request, f"No tables found in database '{sql_db.name}'. Please ensure the database structure has been imported first.")
            return
        ...
        try:
            ...
            def sequential_comment_generation():
                ...
                try:
                    ...
                    column_task_id = start_async_column_comments(workspace.id, column_ids, request.user.id)
                    
                    # Wait for column comments to complete before starting table comments
                    workspace.refresh_from_db()
                    while workspace.column_comment_status == workspace.PreprocessingStatus.RUNNING:
                        time.sleep(5)  # Check every 5 seconds
                        workspace.refresh_from_db()
                    
                    # Step 2: If columns completed successfully, start table comments
                    if workspace.column_comment_status == workspace.PreprocessingStatus.COMPLETED:
                        table_task_id = start_async_table_comments(workspace.id, table_ids, request.user.id)
                    else:
                        # Log that table comments were skipped due to column comment failure
                        workspace.table_comment_log = f"Table comment generation skipped due to column comment failure"
                        workspace.save()
                        
                except Exception as e:
                    workspace.refresh_from_db()
                    workspace.table_comment_log = f"Sequential comment generation error: {str(e)}"
                    workspace.save()
            ...
            workspace.column_comment_task_id = f"sequential_comments_{workspace.id}_{int(time.time())}"
    ...
    generate_all_comments.short_description = "Generate comments for all tables and columns (AI assisted)"
    ...
    def scan_gdpr_compliance(self, request, queryset):
        ...
        try:
            # Check workspace
            if not hasattr(request, 'current_workspace') or not request.current_workspace:
                self.message_user(request, "No active workspace found. Please select a workspace.", messages.ERROR)
                return
            
            # Process only the first database
            if queryset.count() > 1:
                self.message_user(request, "Multiple databases selected. Processing only the first one.", messages.WARNING)
            ...
            # Perform GDPR scan
            self.message_user(request, f"Starting GDPR compliance scan for database '{db.name}'...", messages.INFO)
            
            report = scan_database_for_gdpr(db.id)
            
            if 'error' in report:
                self.message_user(request, f"Error during GDPR scan: {report['error']}", messages.ERROR)
                return
            ...
            self.message_user(request, message, messages.SUCCESS if summary['critical_findings'] == 0 else messages.WARNING)
            
        except Exception as e:
            self.message_user(request, f"An unexpected error occurred during GDPR scan: {str(e)}", messages.ERROR)
