...
class SqlDbAdminForm(forms.ModelForm):
    ...
    class Meta:
        ...
        help_texts = {
            ...
            "informix_server": "INFORMIXSERVER name for Informix databases (e.g., 'ns1i10'). Defaults to db_host if not specified.",
            "informix_protocol": "Connection protocol for Informix (default: onsoctcp). Only used for Informix databases.",
            "informix_dir": "Path to INFORMIXDIR on remote server (default: /u/appl/ids10). Required for Informix SSH connections.",
        }
        widgets = {
    ...
    def clean(self):
        ...
        if cleaned_data.get("db_type") == SQLDBChoices.INFORMIX:
            if not cleaned_data.get("ssh_enabled"):
                self.add_error(
                    "ssh_enabled",
                    "SSH tunnel is required for Informix connections. Informix adapter uses SSH + dbaccess."
                )
    ...
    fieldsets = (
        ...
        (
            "Informix Configuration",
            {
                ...
                "classes": ("collapse",),
                "description": "IBM Informix specific parameters. SSH tunnel is required for Informix databases.",
            },
        ),
        (
            "AI Configuration",
