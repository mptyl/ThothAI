...
class ThothLogAdminForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'sql_generation_failure_message': forms.Textarea(attrs={'rows': 3, 'cols': 80}),
...
class ThothLogAdmin(admin.ModelAdmin):
    ...
    list_display = ('id', 'username', 'workspace', 'formatted_started_at', 'formatted_terminated_at', 'duration', 'get_question_preview')
    ...
    readonly_fields = (
        'username', 'workspace', 'formatted_started_at', 'formatted_terminated_at', 'duration',
        'question', 'db_language', 'question_language', 'translated_question',
        'directives',
        'formatted_keywords_list', 'formatted_evidences', 'formatted_similar_questions', 
        'formatted_similar_columns', 'formatted_schema_with_examples', 'formatted_schema_from_vector_db', 'formatted_reduced_schema', 'formatted_used_mschema',
        'generated_tests_display', 'evaluation_results_display', 'selection_metrics_display', 'pool_of_generated_sql_display', 'selected_sql', 'sql_generation_failure_message', 'sql_explanation',
        'formatted_available_context_tokens', 'formatted_full_schema_tokens_count', 'formatted_schema_link_strategy',
        'formatted_created_at', 'formatted_updated_at'
    )
    
    fieldsets = (
        ('Basic Information', {
            'fields': ('username', 'workspace', 'formatted_started_at', 'formatted_terminated_at', 'duration'),
            'description': 'Basic information about the workflow execution.'
        }),
        ('Question Details', {
            'fields': ('question', 'db_language', 'question_language', 'translated_question', 'directives'),
            'description': 'Original question and language information.'
        }),
        ('Token Management', {
            'fields': ('formatted_available_context_tokens', 'formatted_full_schema_tokens_count', 'formatted_schema_link_strategy'),
            'description': 'Information about token usage and context window management.',
            'classes': ('collapse',)
        }),
        ('Processing Information', {
            'fields': ('formatted_keywords_list', 'formatted_evidences', 'formatted_similar_questions'),
            'description': 'Information extracted and retrieved during processing.',
            'classes': ('collapse',)
        }),
        ('Schema Information', {
            'fields': ('formatted_similar_columns', 'formatted_schema_with_examples', 'formatted_schema_from_vector_db', 'formatted_reduced_schema', 'formatted_used_mschema'),
        ...
        ('Generated Output', {
            'fields': ('pool_of_generated_sql_display', 'selected_sql', 'sql_generation_failure_message', 'sql_explanation'),
    ...
    def pool_of_generated_sql_display(self, obj):
        ...
        try:
            ...
            if isinstance(data, list):
                ...
                for item in data:
                    sql_text = str(item)
                    display_sql = sql_text[:800] + '...' if len(sql_text) > 800 else sql_text
        ...
        html_wrapper += format_html('<pre class="readonly" style="max-height: 300px; overflow-y: auto;">{}</pre>', obj.pool_of_generated_sql)
        html_wrapper += '</div>'
        html_wrapper += '</details>'
        return mark_safe(html_wrapper)
    ...
    def formatted_keywords_list(self, obj):
        ...
        try:
            ...
            if isinstance(keywords, list):
                ...
                html_content += '<div class="readonly" style="margin-top: 10px; padding: 10px;">'
        ...
        html_content = '<details style="border: 1px solid var(--hairline-color, #e0e0e0); border-radius: 4px; padding: 8px; margin: 5px 0;">'
        html_content += '<summary style="cursor: pointer; font-weight: bold; padding: 5px;">Keywords List (click to expand)</summary>'
        html_content += format_html('<pre class="readonly" style="margin-top: 10px; padding: 10px;">{}</pre>', obj.keywords_list)
        html_content += '</details>'
        return mark_safe(html_content)
    ...
    def formatted_evidences(self, obj):
        ...
        try:
            ...
            if isinstance(evidences, list):
                ...
                for i, evidence in enumerate(evidences, 1):
                    ...
                    display_evidence = evidence[:200] + '...' if len(evidence) > 200 else evidence
                    html_content += f'<div>'
        ...
        html_content = '<details style="border: 1px solid var(--hairline-color, #e0e0e0); border-radius: 4px; padding: 8px; margin: 5px 0;">'
        html_content += '<summary style="cursor: pointer; font-weight: bold; padding: 5px;">Evidences (click to expand)</summary>'
        html_content += format_html('<pre class="readonly" style="max-height: 300px; overflow-y: auto; margin-top: 10px; padding: 10px;">{}</pre>', obj.evidences)
        html_content += '</details>'
        return mark_safe(html_content)
    ...
    def formatted_similar_questions(self, obj):
            ...
            if isinstance(questions, list):
                ...
                for i, item in enumerate(questions, 1):
                    if isinstance(item, dict):
                        ...
                        display_question = str(item)[:200] + '...' if len(str(item)) > 200 else str(item)
        ...
        html_content = '<details style="border: 1px solid var(--hairline-color, #e0e0e0); border-radius: 4px; padding: 8px; margin: 5px 0;">'
        html_content += '<summary style="cursor: pointer; font-weight: bold; padding: 5px;">Similar Questions (click to expand)</summary>'
        html_content += format_html('<pre class="readonly" style="max-height: 300px; overflow-y: auto; margin-top: 10px; padding: 10px;">{}</pre>', obj.similar_questions)
        html_content += '</details>'
        return mark_safe(html_content)
    ...
    def formatted_similar_columns(self, obj):
        ...
        try:
            ...
            if isinstance(data, list):
                ...
                for i, item in enumerate(data, 1):
                    if isinstance(item, dict):
                        ...
                        html_content += f'<div style="margin-bottom: 15px; padding: 10px; border: 1px solid var(--hairline-color, #e0e0e0); border-radius: 4px; background: var(--body-bg, #f8f9fa);">'
                        ...
                        html_content += f'<div style="margin-bottom: 5px;">{i}. {item}</div>'
                    else:
                        # Fallback for other types
                        html_content += f'<div style="margin-bottom: 5px;">{i}. {str(item)}</div>'
                html_content += '</div>'
            ...
            html_content += format_html('<pre class="readonly" style="max-height: 400px; overflow-y: auto;">{}</pre>', obj.similar_columns)
    ...
    def formatted_schema_with_examples(self, obj):
        ...
        try:
            ...
            html_content += '<div class="readonly" style="max-height: 500px; overflow-y: auto;">'
            
            if isinstance(data, dict):
                # Likely contains tables as keys with their schemas and examples
                for table_name, table_info in data.items():
                    html_content += f'<div style="margin-bottom: 20px; padding: 15px; border: 1px solid var(--hairline-color, #e0e0e0); border-radius: 4px; background: var(--body-bg, #f8f9fa);">'
                    ...
                    if isinstance(table_info, dict):
                        ...
                        if 'examples' in table_info or 'sample_data' in table_info:
                            examples = table_info.get('examples', table_info.get('sample_data', []))
                            if examples:
                                html_content += '<div style="margin-bottom: 10px;">'
                                ...
                                if isinstance(examples, list):
                                    html_content += json.dumps(examples[:3], indent=2, ensure_ascii=False)  # Show first 3 examples
                                else:
                                    html_content += json.dumps(examples, indent=2, ensure_ascii=False)
                                html_content += '</pre>'
                                html_content += '</div>'
            ...
            # Fallback to raw display if not valid JSON
            html_content += format_html('<pre class="readonly" style="max-height: 400px; overflow-y: auto;">{}</pre>', obj.schema_with_examples)
    ...
    def formatted_schema_from_vector_db(self, obj):
        ...
        try:
            ...
            if isinstance(data, list):
                ...
                for i, item in enumerate(data, 1):
                    html_content += f'<div style="margin-bottom: 15px; padding: 15px; border: 1px solid var(--hairline-color, #e0e0e0); border-radius: 4px; background: var(--body-bg, #f8f9fa);">'
                    ...
                    if isinstance(item, dict):
                        ...
                        displayed_keys = ['table_name', 'table', 'column_name', 'column', 'description', 'desc', 'data_type', 'type']
                        for key, value in item.items():
                            if key not in displayed_keys:
                                display_key = key.replace('_', ' ').title()
                                if isinstance(value, (list, dict)):
                                    html_content += f'<div style="margin-top: 8px;">'
                                    html_content += f'<strong>{display_key}:</strong>'
                                    html_content += f'<pre class="readonly" style="margin: 5px 0 0 15px; padding: 8px; border-radius: 4px; font-size: 0.9em;">{json.dumps(value, indent=2, ensure_ascii=False)}</pre>'
                                    html_content += '</div>'
                                else:
            ...
            # Fallback to raw display if not valid JSON
            html_content += format_html('<pre class="readonly" style="max-height: 400px; overflow-y: auto;">{}</pre>', obj.schema_from_vector_db)
    ...
    def formatted_reduced_schema(self, obj):
        ...
        html_content += '<div style="margin-top: 10px; padding: 10px;">'
        html_content += format_html('<pre class="readonly" style="max-height: 400px; overflow-y: auto;">{}</pre>', obj.reduced_schema)
        html_content += '</div>'
        html_content += '</details>'
        return mark_safe(html_content)
    ...
    def formatted_used_mschema(self, obj):
        ...
        html_content += '<div style="margin-top: 10px; padding: 10px;">'
        html_content += format_html('<pre class="readonly" style="max-height: 400px; overflow-y: auto;">{}</pre>', obj.used_mschema)
        html_content += '</div>'
        html_content += '</details>'
        return mark_safe(html_content)
    ...
    def generated_tests_display(self, obj):
                ...
                for i, item in enumerate(data, 1):
                    ...
                    if answers:
                        ...
                        for j, ans in enumerate(answers, 1):
                            ans_text = str(ans)
                            display_ans = ans_text[:600] + '...' if len(ans_text) > 600 else ans_text
            ...
            if isinstance(data, list):
                ...
                for i, item in enumerate(data, 1):
                    ...
                    if isinstance(item, (list, tuple)) and len(item) >= 2:
                        thinking = item[0]
                        ans = item[1]
                        answers = list(ans) if isinstance(ans, (list, tuple)) else [ans] if ans else []
                    else:
                        thinking = str(item)
                        answers = []
                    ...
                    # Test Generator Section
                    html_content += '<div style="margin-left: 18px; margin-bottom: 8px;">'
                    ...
                    html_content += '<div style="margin-left: 12px;"><strong>Unit Tests:</strong>'
                    if answers:
                        html_content += '<ul style="margin: 6px 0 0 18px;">'
                        for ans in answers:
                            ans_text = str(ans)
        ...
        # 3) Final fallback: raw text wrapped in expandable container
        html_wrapper += format_html('<pre class="readonly" style="max-height: 300px; overflow-y: auto;">{}</pre>', obj.generated_tests)
        html_wrapper += '</div>'
        html_wrapper += '</details>'
        return mark_safe(html_wrapper)
    ...
    def evaluation_results_display(self, obj):
        ...
        try:
            ...
            if isinstance(data, list) and len(data) > 0:
                ...
                has_enhanced_metadata = any(isinstance(item, dict) and item.get('evaluation_type') == 'enhanced' for item in data)
            ...
            if isinstance(data, list) and len(data) >= 2 and isinstance(data[0], str) and isinstance(data[1], list):
                # New format: single tuple [thinking, [verdicts]] or [thinking, [verdicts], [test_units]]
                thinking = data[0]
                verdicts = data[1]
                test_units = data[2] if len(data) > 2 and isinstance(data[2], list) else []
                ...
                if verdicts:
                    ...
                    html_content += '<ul style="margin: 5px 0 0 20px; list-style: none;">'
                    for j, verdict in enumerate(verdicts, 1):
                        ...
                        for part in test_results_str.split(','):
                            part = part.strip()
                            if part.startswith('OK') or (current_result and not current_result.startswith('KO')):
                                if current_result:
                        ...
                        # Count failures
                        failure_count = sum(1 for r in test_results if r.startswith('KO'))
                        ...
                        html_content += f'<li style="margin: 8px 0;">'
                ...
                if test_units:
                    ...
                    for test in test_units:
                        html_content += f'<li style="margin: 3px 0; font-size: 0.9em;">{test}</li>'
                    html_content += '</ol>'
        ...
        try:
            ...
            if isinstance(data, (tuple, list)) and len(data) >= 2 and isinstance(data[0], str) and isinstance(data[1], list):
        ...
        html_wrapper += format_html('<pre class="readonly" style="max-height: 300px; overflow-y: auto;">{}</pre>', obj.evaluation_results)
        html_wrapper += '</div>'
        html_wrapper += '</details>'
        return mark_safe(html_wrapper)
    ...
    def _render_enhanced_evaluation_results(self, data, html_wrapper):
        ...
        metadata = next((item for item in data if isinstance(item, dict) and item.get('evaluation_type') == 'enhanced'), None)
        ...
        if metadata:
            ...
            if processing_time:
                html_content += f'<p><b>⏱️ Processing Time:</b> {processing_time:.1f}ms</p>'
            
            # Show escalation info if applicable
            requires_escalation = metadata.get('requires_escalation', False)
            if requires_escalation:
                html_content += f'<p><b>🚨 Requires Escalation:</b> Yes</p>'
            
            # Show auxiliary agents used
            aux_agents = metadata.get('auxiliary_agents_used', [])
            if aux_agents:
                html_content += f'<p><b>🤖 Auxiliary Agents Used:</b> {", ".join(aux_agents)}</p>'
            
            # Show best pass rate
            best_pass_rate = metadata.get('best_pass_rate', 0)
            if best_pass_rate > 0:
                html_content += f'<p><b>📊 Best Pass Rate:</b> {best_pass_rate:.1f}%</p>'
            
            html_content += '</div>'
        
        # Show individual SQL evaluation results
        sql_results = [item for item in data if isinstance(item, dict) and 'sql_index' in item]
        if sql_results:
            html_content += '<h5 style="margin: 15px 0 10px 0;">SQL Candidate Results:</h5>'
    ...
    def selection_metrics_display(self, obj):
        ...
        try:
            ...
            if sql_scores:
                html_content += '<h5 style="margin: 15px 0 10px 0;">SQL Candidate Scores:</h5>'
                ...
                for score in sql_scores:
                    ...
                    html_content += f'<div style="margin: 10px 0; padding: 10px; border: 1px solid var(--hairline-color, #e0e0e0); border-radius: 5px;">'
                    ...
                    if test_details:
                        ...
                        html_content += '<summary style="cursor: pointer;">Test Details</summary>'
                        html_content += '<ul style="margin: 5px 0 0 20px; font-size: 0.9em;">'
                    ...
                    if failure_reasons:
                        ...
                        html_content += '<ul style="margin: 5px 0 0 30px; font-size: 0.9em;">'
                        for failure in failure_reasons:
                            # Handle new format: failure reasons are now strings
                            if isinstance(failure, str):
                                html_content += f'<li style="margin: 5px 0;">'
                                ...
                                html_content += f'<li style="margin: 5px 0;">'
                                html_content += f'<b>Test {test_num}:</b> {test_desc}<br>'
            ...
            if test_units:
                html_content += '<h5 style="margin: 15px 0 10px 0;">Test Units Applied:</h5>'
            ...
            # Raw data toggle
            html_content += f'<details style="margin-top: 15px;"><summary style="cursor: pointer;">Show raw JSON</summary>'
        ...
        html_wrapper += format_html('<pre class="readonly" style="max-height: 300px; overflow-y: auto;">{}</pre>', obj.selection_metrics)
        html_wrapper += '</div>'
        html_wrapper += '</details>'
        return mark_safe(html_wrapper)
    ...
    def _render_enhanced_selection_metrics(self, data, html_wrapper):
        ...
        html_content += f'<p style="margin: 5px 0;"><b>🎯 Evaluation Case:</b> {case}</p>'
        ...
        if pass_rates:
            ...
            for sql_key, rate in pass_rates.items():
                border_color = '#28a745' if rate >= 100 else '#ffc107' if rate >= 90 else '#dc3545'
        ...
        if aux_agents:
            html_content += '<h5 style="margin: 15px 0 10px 0;">Auxiliary Agents Used:</h5>'
        ...
        if case == 'B' and 'sql_selection' in data:
            ...
            assessment = deep_eval.get('supervisor_assessment', 'No assessment provided')
        ...
        if escalation_info.get('requires_escalation', False):
            html_content += '<h5 style="margin: 15px 0 10px 0;">Escalation Information:</h5>'
        ...
        if session_logs:
            ...
            html_content += f'<p><b>Session ID:</b> {session_logs.get("session_id", "Unknown")}</p>'
            html_content += f'<p><b>Total Events:</b> {session_logs.get("total_events", 0)}</p>'
            ...
            if event_types:
                ...
                for event_type, count in event_types.items():
                    if count > 0:
                        html_content += f'<li>{event_type.replace("_", " ").title()}: {count}</li>'
                html_content += '</ul>'
    ...
    def formatted_full_schema_tokens_count(self, obj):
        ...
        if obj.full_schema_tokens_count is not None:
            ...
            if obj.available_context_tokens and obj.available_context_tokens > 0:
                percentage = (obj.full_schema_tokens_count / obj.available_context_tokens) * 100
                
                return format_html(
                    '<div style="font-family: monospace; font-size: 1.1em;">'
                    '<strong>{}</strong> tokens ({}%)'
                    '</div>',
                    tokens, f'{percentage:.1f}'
                )
            else:
