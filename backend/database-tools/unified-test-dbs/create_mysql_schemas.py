#!/usr/bin/env python3
"""
Create MySQL schemas with proper table structures and MySQL-specific configurations
"""
...
def sqlite_type_to_mysql(sqlite_type: str, is_primary_key: bool = False) -> str:
...
def quote_identifier(name: str) -> str:
    ...
    # MySQL reserved words that need quoting
    reserved_words = {
        ...
        'in', 'exists', 'between', 'like', 'similar', 'is', 'key', 'index',
        'primary', 'foreign', 'references', 'constraint', 'check', 'default',
        'virtual', 'rank', 'status', 'position', 'time', 'date', 'year'
    }
...
def create_mysql_table(db_name: str, table_name: str, sqlite_path: str, descriptions_path: str):
    """Create a MySQL table from SQLite schema"""
    ...
    if table_exists:
        ...
        mysql_cursor.execute(f"DROP TABLE {quote_identifier(table_name)}")
        mysql_conn.commit()
    ...
    for col in columns:
        col_name = col[1]
        is_pk = col[5]
        col_type = sqlite_type_to_mysql(col[2], is_pk)
        ...
        if col_name in descriptions:
            # Escape single quotes and other problematic characters in descriptions for MySQL
            safe_description = descriptions[col_name].replace("'", "''").replace('"', '""').replace('\n', ' ').replace('\r', ' ')
            # Limit comment length to avoid MySQL limits
...
def main():
    ...
    for db_name, db_config in config['databases'].items():
        ...
        print(f"\nProcessing MySQL database: {db_name}")
        ...
        for table_name in tables:
            ...
            if table_name == 'sqlite_sequence':
                ...
                continue
            create_mysql_table(db_name, table_name, sqlite_path, descriptions_path)
