    ...
    for attempt in range(max_attempts):
        try:
            ...
            if attempt < max_attempts - 1:
                print(
                    f"⏳ Waiting for Supabase... (attempt {attempt + 1}/{max_attempts})"
                )
                time.sleep(2)
            else:
                print(
                    f"[ERROR] Failed to connect to Supabase after {max_attempts} attempts: {e}"
                )
                return False
    return False
    ...
    for db_name in databases:
        try:
            ...
            if db_name in test_queries:
                ...
                print(
                    f"[SUCCESS] {db_name}: Connected successfully, found {count} records"
                )
            else:
