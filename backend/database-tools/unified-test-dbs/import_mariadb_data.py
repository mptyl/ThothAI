...
def import_table_data(db_name: str, table_name: str, sqlite_path: str):
    ...
    if len(sqlite_column_names) != len(mariadb_column_names):
        print(f"  ✗ Column count mismatch: SQLite has {len(sqlite_column_names)}, MariaDB has {len(mariadb_column_names)}")
        sqlite_conn.close()
        mariadb_conn.close()
        return
            ...
            try:
                ...
                for j, row in enumerate(batch):
                    try:
                        ...
                        print(f"      Row data length: {len(row)}, expected: {len(mariadb_column_names)}")
                        mariadb_conn.rollback()
