...
"""
Import data from SQLite to MariaDB with proper column name handling
"""
...
def import_table_data(db_name: str, table_name: str, sqlite_path: str):
    """Import data from SQLite table to MariaDB table"""
    ...
    # Get MariaDB column names in the correct order
    mariadb_cursor.execute(f"""
        SELECT COLUMN_NAME 
        FROM INFORMATION_SCHEMA.COLUMNS 
        WHERE TABLE_SCHEMA = '{db_name}' AND TABLE_NAME = '{table_name}'
        ORDER BY ORDINAL_POSITION
    """)
    mariadb_column_names = [row[0] for row in mariadb_cursor.fetchall()]

    # Quote the MariaDB column names for the INSERT statement
    quoted_columns = [quote_identifier(name) for name in mariadb_column_names]

    print(f"  SQLite columns: {len(sqlite_column_names)}")
    print(f"  MariaDB columns: {len(mariadb_column_names)}")

    if len(sqlite_column_names) != len(mariadb_column_names):
        print(
            f"  ✗ Column count mismatch: SQLite has {len(sqlite_column_names)}, MariaDB has {len(mariadb_column_names)}"
        )
        sqlite_conn.close()
        mariadb_conn.close()
        return
    ...
    try:
        # Clear existing data
        mariadb_cursor.execute(f"DELETE FROM {quote_identifier(table_name)}")

        # Prepare insert statement
        placeholders = ", ".join(["%s"] * len(mariadb_column_names))
        insert_sql = (
        ...
        for i in range(0, len(rows), batch_size):
            ...
            try:
                ...
                mariadb_conn.commit()
                print(
                    f"    Inserted batch {i // batch_size + 1}/{(len(rows) + batch_size - 1) // batch_size}"
                )
                ...
                for j, row in enumerate(batch):
                    try:
                        ...
                        print(
                            f"      Row data length: {len(row)}, expected: {len(mariadb_column_names)}"
                        )
                        mariadb_conn.rollback()
                print(
                    f"    Successfully inserted {success_count}/{len(batch)} rows from batch"
                )
...
def main():
    ...
    for db_name, db_config in config["databases"].items():
        ...
        print(f"\nImporting data to MariaDB database: {db_name}")
