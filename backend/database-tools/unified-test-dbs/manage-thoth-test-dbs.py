...
def start_services():
    ...
    result = run_command(
        ["docker-compose", "-f", COMPOSE_FILE, "up", "-d"], capture_output=False
    )
...
def check_health():
    ...
    try:
        response = requests.get(
            f"http://localhost:{config['shared']['adminer_port']}", timeout=5
        )
    ...
    if config["servers"]["supabase"]["enabled"]:
        try:
            response = requests.get(
                f"http://localhost:{config['servers']['supabase']['rest_port']}",
                timeout=5,
            )
            if response.status_code in [
                ...
                400,
                404,
            ]:  # 400 is normal for missing auth roles, 404 is normal for root path
...
def show_logs(service: str = None):
    ...
    if service:
        ...
        run_command(
            ["docker-compose", "-f", COMPOSE_FILE, "logs", "-f"], capture_output=False
        )
...
def setup_all():
    ...
    if config["servers"]["supabase"]["enabled"]:
        print(
            f"- Supabase REST API: http://localhost:{config['servers']['supabase']['rest_port']}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Manage thoth-test-dbs unified database pool"
    )
    parser.add_argument(
        "command",
        choices=[
