#!/usr/bin/env python3
"""
Create Supabase schemas with proper table structures and Supabase-specific configurations
"""
...
def create_supabase_table(db_name: str, table_name: str, sqlite_path: str, descriptions_path: str):
    """Create a Supabase table from SQLite schema with Supabase-specific features"""
    ...
    print(f"Creating Supabase table {table_name}...")
    ...
    try:
        ...
        for col_name, description in descriptions.items():
            ...
            comment_sql = f"COMMENT ON COLUMN {table_name}.{quoted_name} IS '{safe_description}';"
            try:
        ...
        try:
            pg_cursor.execute(f"ALTER TABLE {table_name} ENABLE ROW LEVEL SECURITY;")
            pg_conn.commit()
            print(f"✓ RLS enabled for {table_name}")
        except Exception as e:
            print(f"Warning: Could not enable RLS for {table_name}: {e}")
            pg_conn.rollback()
        
        # Create a permissive policy for now (can be customized later)
        try:
            policy_sql = f"""
            CREATE POLICY "Allow all operations" ON {table_name}
            FOR ALL USING (true) WITH CHECK (true);
            """
            pg_cursor.execute(policy_sql)
            pg_conn.commit()
            print(f"✓ Permissive policy created for {table_name}")
        except Exception as e:
            print(f"Warning: Could not create policy for {table_name}: {e}")
            pg_conn.rollback()
        
        print(f"✓ Comments and Supabase features configured for {table_name}")
...
def main():
    ...
    for db_name, db_config in config['databases'].items():
        ...
        print(f"\nProcessing Supabase database: {db_name}")
        ...
        for table_name in tables:
            ...
            if table_name == 'sqlite_sequence':
                ...
                continue
            create_supabase_table(db_name, table_name, sqlite_path, descriptions_path)
