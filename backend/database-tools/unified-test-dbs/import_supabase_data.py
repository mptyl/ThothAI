...
"""
Import data from SQLite to Supabase with proper column name handling
"""
...
def import_table_data(db_name: str, table_name: str, sqlite_path: str):
    """Import data from SQLite table to Supabase table"""
    ...
    # Get Supabase column names in the correct order
    ...
    # Quote the Supabase column names for the INSERT statement
    quoted_columns = [quote_identifier(name) for name in pg_column_names]

    print(f"  SQLite columns: {len(sqlite_column_names)}")
    print(f"  Supabase columns: {len(pg_column_names)}")

    if len(sqlite_column_names) != len(pg_column_names):
        print(
            f"  ✗ Column count mismatch: SQLite has {len(sqlite_column_names)}, Supabase has {len(pg_column_names)}"
        )
        sqlite_conn.close()
        pg_conn.close()
        return
            ...
            try:
                ...
                for j, row in enumerate(batch):
                    try:
                        ...
                        print(
                            f"      Row data length: {len(row)}, expected: {len(pg_column_names)}"
                        )
                        pg_conn.rollback()
                print(
...
def main():
    ...
    for db_name, db_config in config["databases"].items():
        ...
        print(f"\nImporting data to Supabase database: {db_name}")
