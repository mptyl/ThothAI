...
def generate_mariadb_schema(
    db_name: str, schemas: List[Tuple], descriptions_by_table: Dict[str, Dict[str, str]]
) -> str:
    ...
    for table_name, columns in schemas:
        ...
        for col_info in columns:
            ...
            comment = ""
            if (
                table_name in descriptions_by_table
                and col_name in descriptions_by_table[table_name]
            ):
                ...
                for i, col_def in enumerate(column_defs):
                    if col_def.startswith(pk_columns[0]):
                        column_defs[i] = col_def.replace(
                            col_def.split()[1], col_def.split()[1] + " PRIMARY KEY", 1
                        )
                        break
            else:
...
def generate_postgres_schema(
    ...
    for table_name, columns in schemas:
        ...
        pk_columns = [
            f'"{col[1]}"'
            if " " in col[1] or "(" in col[1] or ")" in col[1] or "%" in col[1]
            else col[1]
            for col in columns
            if col[5]
        ]
...
def generate_sqlserver_schema(
    ...
    for table_name, columns in schemas:
        ...
        if table_name in descriptions_by_table:
            for col_name, description in descriptions_by_table[table_name].items():
                sql += "EXEC sp_addextendedproperty @name = N'MS_Description', "
...
def generate_oracle_schema(
    ...
    for table_name, columns in schemas:
        ...
        if table_name in descriptions_by_table:
            for col_name, description in descriptions_by_table[table_name].items():
                sql += (
                    f"COMMENT ON COLUMN {table_name}.{col_name} IS '{description}';\\n"
                )
            sql += "\\n"
...
def main():
    ...
    for db_name, db_config in config["databases"].items():
        ...
        if os.path.exists(description_dir):
            for csv_file in os.listdir(description_dir):
                if csv_file.endswith(".csv"):
                    ...
                    descriptions_by_table[table_name] = read_column_descriptions(
                        csv_path
                    )
        ...
        if servers["mariadb"]["enabled"]:
            mariadb_sql = generate_mariadb_schema(
                db_name, schemas, descriptions_by_table
            )
        ...
        if servers["postgres"]["enabled"]:
            postgres_sql = generate_postgres_schema(
                db_name, schemas, descriptions_by_table
            )
        ...
        sqlserver_sql = generate_sqlserver_schema(
            db_name, schemas, descriptions_by_table
        )
        ...
        if servers["supabase"]["enabled"]:
            supabase_sql = generate_supabase_schema(
                db_name, schemas, descriptions_by_table
            )
