...
SQLITE_DB_PATH = (
    "/Users/mp/thoth_data/dev_databases/california_schools/california_schools.sqlite"
)
DESCRIPTIONS_DIR = (
...
def connect_mariadb() -> mysql.connector.MySQLConnection:
    ...
    try:
        ...
        logger.info(
            f"Connected to MariaDB database: {MARIADB_CONFIG['host']}:{MARIADB_CONFIG['port']}"
        )
        return conn
...
def import_schools_data(
    sqlite_conn: sqlite3.Connection, mariadb_conn: mysql.connector.MySQLConnection
) -> int:
...
def import_frpm_data(
    ...
    try:
        ...
        placeholders = ", ".join(["%s"] * len(columns))
        insert_query = (
            f"INSERT INTO frpm ({', '.join(columns)}) VALUES ({placeholders})"
        )
...
def import_satscores_data(
    ...
    try:
        ...
        placeholders = ", ".join(["%s"] * len(columns))
        insert_query = (
            f"INSERT INTO satscores ({', '.join(columns)}) VALUES ({placeholders})"
        )
        ...
        count = len(data_tuples)
        logger.info(
            f"Successfully imported {count} SAT scores records (filtered orphaned records)"
        )
        return count
...
def verify_import(mariadb_conn: mysql.connector.MySQLConnection) -> None:
    ...
    try:
        ...
        for row in cursor.fetchall():
            logger.info(
                f"  {row[0]} ({row[1]}, {row[2]}) - Enrollment: {row[3]}, SAT Takers: {row[4]}"
            )
...
def main():
    ...
    try:
        ...
        logger.info(
            f"Total records imported: {schools_count + frpm_count + satscores_count}"
        )
        logger.info(f"  - Schools: {schools_count}")
        logger.info(f"  - FRPM: {frpm_count}")
        logger.info(f"  - SAT Scores: {satscores_count}")
        logger.info(
            "Column descriptions are embedded in the schema as COMMENT attributes"
        )
