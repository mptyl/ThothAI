...
def merge_pyproject_files(
    ...
    try:
        ...
        if local_path.exists():
            ...
            print(f"[SUCCESS] Merged {base_path} with {local_path}")
        else:
            # No local overrides, use base only
            merged = base_config
            print("[INFO] No local overrides found, using base configuration only")

        # Write merged configuration
        with open(output_path, "w") as f:
            toml.dump(merged, f)

        print(f"[SUCCESS] Merged configuration written to {output_path}")
        return True

    except Exception as e:
        print(f"[ERROR] Error merging TOML files: {e}")
        return False
...
def create_local_config_from_chosen_db(
    ...
    try:
        ...
        if not chosen_db_path.exists():
            print(f"[INFO] No database selection file found at {chosen_db_path}")
        ...
        with open(local_path, "w") as f:
            if local_config:
                ...
                print(f"[SUCCESS] Created {local_path} with database selections")
            else:
                # Empty file for default (PostgreSQL + SQLite only)
                f.write(
                    "# Using default database configuration (PostgreSQL + SQLite)\n"
                )
                print(f"[SUCCESS] Created {local_path} with default configuration")

        return True

    except Exception as e:
        print(f"[ERROR] Error creating local config: {e}")
        return False
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("Usage:")
        print(
            "  python toml_merger.py merge         # Merge pyproject.toml with local overrides"
        )
        print(
            "  python toml_merger.py create-local  # Create local config from chosen_db.json"
        )
        sys.exit(1)
