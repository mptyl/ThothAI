...
from thoth_ai_backend.preprocessing.update_database_columns_direct import (
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.NOTICE(
                f"Starting column description update for workspace ID: {workspace_id}"
            )
        )
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully updated column descriptions for workspace ID: {workspace_id}"
                )
            )
            logger.info(
                f"Successfully updated column descriptions for workspace ID: {workspace_id} via management command."
            )
            ...
            self.stderr.write(
                self.style.ERROR(
                    f"File not found during update for workspace ID {workspace_id}: {str(e)}"
                )
            )
            ...
            self.stderr.write(
                self.style.ERROR(
                    f"Value error during update for workspace ID {workspace_id}: {str(e)}"
                )
            )
            ...
            self.stderr.write(
                self.style.ERROR(
                    f"An unexpected error occurred for workspace ID {workspace_id}: {str(e)}"
                )
            )
            logger.error(
                f"Unexpected error for workspace ID {workspace_id}: {str(e)}",
                exc_info=True,
            )
