...
def run_preprocessing_task(workspace_id):
    ...
    try:
        ...
        sql_db_params["vector_db"] = {
            ...
            "username": vector_db_obj.username, "password": vector_db_obj.password,
        ...
        if plugin_db_type == 'sqlite':
            ...
            sqlite_dir = os.path.join(db_root_path, f"{sql_db_params['db_mode']}_databases", sql_db_params['db_name'])
            connection_params['database_path'] = os.path.join(sqlite_dir, f"{sql_db_params['db_name']}.sqlite")
        else:
        ...
        try:
            if hasattr(vector_db, 'delete_collection'):
                ...
                logger.info(f"Deleted existing COLUMN_NAME collection for preprocessing")
