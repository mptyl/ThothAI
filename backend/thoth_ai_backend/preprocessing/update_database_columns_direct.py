...
import logging
from thoth_core.models import SqlTable, SqlColumn, Workspace
...
def update_database_columns_description(workspace_id):
    ...
    try:
        ...
        logging.info(
            f"Using database '{database_name}' (mode: {db_mode_value}) from workspace '{workspace.name}' (ID: {workspace_id})"
        )
        ...
        if not os.path.exists(base_dir):
            error_msg = "Data directory for column descriptions not found. . "
            ...
            if os.path.exists(data_root_full_path) and os.path.isdir(
                data_root_full_path
            ):
                ...
                for mode_subdir_name in os.listdir(data_root_full_path):
                    mode_subdir_path = os.path.join(
                        data_root_full_path, mode_subdir_name
                    )
                    if os.path.isdir(mode_subdir_path):
                        ...
                        for db_name_candidate in os.listdir(mode_subdir_path):
                            db_candidate_description_path = os.path.join(
                                mode_subdir_path,
                                db_name_candidate,
                                "database_description",
                            )
                ...
                with open(file_path, "r", encoding=result["encoding"]) as csvfile:
                    ...
                    for row_num, row in enumerate(
                        ...
                        original_column_name = (
                            original_column_name_raw.lower().strip()
                            if not original_column_name_raw.__contains__(" ")
                            else original_column_name_raw.strip()
                        )
                        ...
                        try:
                            column = SqlColumn.objects.get(
                                original_column_name__iexact=original_column_name,
                                sql_table=table,
                            )

                            # Update column fields
                            column.column_name = row.get(
                                "column_name", column.column_name
                            )  # Keep existing if not provided
                            column.column_description = row.get(
                                "column_description", column.column_description
                            )  # Keep existing
                            column.value_description = row.get(
                            ...
                            column.save()
                            logging.info(
                                f"Successfully updated column '{original_column_name}' in table '{table_name}'"
                            )
        ...
        if processing_errors:
            raise Exception(
                "Errors occurred during column description update:\n- "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Update database column metadata from CSV files"
    )
    parser.add_argument(
        "--workspace_id",
        type=int,
        required=True,
        help="Workspace ID to determine the database",
    )
