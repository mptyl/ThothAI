...
def upload_questions_to_vectordb(workspace_id=None) -> tuple:
    ...
    try:
        if hasattr(vector_db, 'delete_collection'):
            ...
            logging.info(f"Deleted existing SQL collection for {vector_db_config.get('collection_name')}")
    ...
    total_items = sum(1 for entry in dev_data if entry.get("db_id") == db_id and entry.get("question") and isinstance(entry.get("question"), str) and entry.get("SQL") and isinstance(entry.get("SQL"), str))
    ...
    for entry in dev_data:
        ...
        if not question or not isinstance(question, str) or not sql or not isinstance(sql, str):
        ...
        try:
            ...
            ProgressTracker.update_progress(workspace_id, 'questions', processed_items, successful_uploads, failed_uploads)
            progress_percentage = int((processed_items / total_items) * 100) if total_items > 0 else 0
            logging.info(f"Successfully uploaded question ({processed_items}/{total_items} - {progress_percentage}%): {question[:100]}...")
        except Exception as e:
            failed_uploads += 1
            processed_items += 1
            # Update progress tracker
            ProgressTracker.update_progress(workspace_id, 'questions', processed_items, successful_uploads, failed_uploads)
            logging.error(f"Failed to upload question: {question[:100]}... Error: {str(e)}")
    
    # Update the last_sql_loaded timestamp in the workspace
    workspace.last_sql_loaded = timezone.now()
    workspace.save()
    logging.info(f"Updated last_sql_loaded timestamp for workspace {workspace.name} (ID: {workspace_id})")
    
    # Log summary
    logging.info(f"Question upload complete. Successful: {successful_uploads}, Failed: {failed_uploads}, Total: {total_items}")
