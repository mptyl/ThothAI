...
from .backend_utils.vector_store_utils import (
    get_vector_store,
    export_evidence_to_csv_file, export_columns_to_csv_file, export_questions_to_csv_file,
    import_evidence_from_csv_file, import_columns_from_csv_file, import_questions_from_csv_file,
    delete_all_evidence_from_vector_store, delete_all_columns_from_vector_store, delete_all_questions_from_vector_store
)
...
class DbDocsView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            doc_path = os.path.join(settings.BASE_DIR, exports_dir, db_name, f"{db_name}_documentation.html")
            ...
            if os.path.exists(doc_path):
                ...
                context['doc_exists'] = False
                context['doc_content'] = None
                context['info'] = f"Documentation has not been generated yet for database '{db_name}'."
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            if sql_db.gdpr_report:
                ...
                context['report_content'] = None
                context['report_exists'] = False
                context['info'] = f"GDPR compliance scan has not been performed yet for database '{sql_db.name}'."
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            if not sql_db.erd or not sql_db.erd.strip():
                ...
                context['erd_image_path'] = None
                context['info'] = f"ERD diagram has not been generated yet for database '{db_name}'. Please generate database documentation first."
                return context
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            try:
                ...
                context['last_evidence_load'] = workspace.last_evidence_load if workspace else None
...
def manage_evidence(request, evidence_id=None):
        ...
        if evidence_id:
            ...
            try:
                ...
                if not isinstance(instance, EvidenceDocument):
                    logger.error(f"Document found for ID {evidence_id} is not a EvidenceDocument.")
                ...
                logger.error(f"Error fetching evidence {evidence_id}: {e}", exc_info=True)
                raise Http404("Error fetching evidence") # Raise Http404 for consistency on failure
    except Exception as e:
    ...
    if request.method == 'POST':
        form = EvidenceForm(request.POST) # Pass initial data for update if needed, but POST overrides
        if form.is_valid():
            ...
            try:
                if instance: # Update existing evidence
                    ...
                    vector_store.delete_document(instance.id) # Assuming delete_evidence exists
                    vector_store.add_evidence(EvidenceDocument(id=instance.id, evidence=evidence_text)) # Re-add with same ID if possible, or let it generate new
                    ...
                    logger.info(f"New evidence created.")

                return redirect('thoth_ai_backend:evidence') # Redirect after success

            except Exception as e:
                # Log the error e
                logger.error(f"Error saving evidence (ID: {evidence_id}): {e}", exc_info=True)
                # Add error to form or context if needed
                form.add_error(None, f"An error occurred while saving the evidence. {e}") # Generic error
...
def confirm_delete_evidence(request, evidence_id):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Error fetching evidence {evidence_id} for deletion confirmation: {e}", exc_info=True)
...
def delete_evidence_confirmed(request, evidence_id):
    ...
    try:
        ...
        try:
            ...
            messages.success(request, f"Evidence successfully deleted.") # Optional user feedback
            ...
            messages.error(request, "An error occurred while deleting the evidence.") # Optional user feedback
    except Exception as e:
        logger.error(f"Error getting vector store: {e}", exc_info=True)
        messages.error(request, f"Error accessing vector database: {e}")
...
def manage_question(request, question_id=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                if instance: # Update existing question
                    ...
                    vector_store.add_sql(SqlDocument(id=instance.id, question=question_text, sql=sql_query, evidence=evidence_text))
                    ...
                    new_question = SqlDocument(question=question_text, sql=sql_query, evidence=evidence_text)
        ...
        if instance: # Populate form for update
            form = SqlDocumentForm(initial={'question': instance.question, 'sql': instance.sql, 'evidence': instance.evidence})
        else: # Empty form for create
...
def run_preprocessing(request, workspace_id):
    ...
    return render(request, 'partials/preprocessing_progress.html', {
        'workspace': workspace,
...
def upload_evidences(request, workspace_id):
    ...
    try:
        ...
        if successful_uploads > 0:
            context['success_message'] = f"Successfully uploaded {successful_uploads} of {total_items} evidence items."
        ...
        logger.error(f"Error uploading evidence for workspace {workspace_id}: {e}", exc_info=True)
        context['error_message'] = str(e)
    except Exception as e:
        logger.error(f"Unexpected error uploading evidence for workspace {workspace_id}: {e}", exc_info=True)
...
def upload_questions(request, workspace_id):
    ...
    try:
        ...
        if successful_uploads > 0:
            context['success_message'] = f"Successfully uploaded {successful_uploads} of {total_items} questions."
            context['total_items'] = total_items
            context['successful_uploads'] = successful_uploads
        else:
...
def export_evidence_csv(request):
    try:
        ...
        messages.success(request, f"Evidence exported successfully. Saved on server at: {saved_filepath}. Download started.")
        return response
...
def import_evidence_server_csv(request):
    try:
        ...
        messages.error(request, f"An unexpected error occurred during evidence import: {e}")
    return HttpResponseRedirect(reverse('thoth_ai_backend:evidence'))
...
def delete_all_evidence(request):
    ...
    try:
        ...
        logger.error(f"NotImplementedError in delete_all_evidence view: {e}", exc_info=True)
        messages.error(request, f"Could not delete evidence: The delete operation is not implemented correctly. {e}")
    except Exception as e:
...
def export_pdf(request):
    ...
    try:
        ...
        doc = SimpleDocTemplate(buffer, pagesize=A4,
                                rightMargin=36, leftMargin=36,  # Reduced from 72 to 36 (0.5 inch)
        ...
        if sql_db.scope_json:
            try:
                ...
                for key, value in scope_data.items():
                    ...
                    elements.append(Paragraph(f"<b>{friendly_title}</b>", heading2_style))
                    elements.append(Paragraph(value, normal_style))
                    elements.append(Spacer(1, 0.1*inch))
            except json.JSONDecodeError:
                elements.append(Paragraph(sql_db.scope_json, normal_style))
        else:
            elements.append(Paragraph("No scope defined for this database.", normal_style))
        ...
        erd_path = os.path.join(settings.BASE_DIR, io_dir, db_name, f"{db_name}_documentation_erd.png")
        ...
        for table in tables:
            ...
            for column in sorted_columns:
                ...
                description = column.column_description or column.generated_comment or ""
            ...
            t = Table(table_data, colWidths=[1.5*inch, 0.9*inch, 4.6*inch, 0.5*inch])
        ...
        ).select_related('source_table', 'source_column', 'target_table', 'target_column').order_by(
            'source_table__name', 'source_column__original_column_name'
        )
        
        if relationships.exists():
            # Prepare relationship table data
            rel_table_data = [['Source Table', 'Source Column', 'Target Table', 'Target Column']]
            ...
            rel_table = Table(rel_table_data, colWidths=[2*inch, 2*inch, 2*inch, 1.5*inch])
            ...
            elements.append(Paragraph("No foreign key relationships defined in this database.", normal_style))
        ...
        response['Content-Disposition'] = f'attachment; filename="{db_name}_documentation.pdf"'
...
def gdpr_export_pdf(request):
    ...
    try:
        ...
        if not sql_db.gdpr_report:
            messages.error(request, f"GDPR compliance scan has not been performed yet for database '{db_name}'.")
            return redirect('thoth_ai_backend:gdpr_report')
        ...
        # Title
        elements.append(Paragraph(f"GDPR Compliance Report", title_style))
        elements.append(Paragraph(f"Database: {db_name}", heading_style))
        
        if sql_db.gdpr_scan_date:
            elements.append(Paragraph(f"Scan Date: {sql_db.gdpr_scan_date.strftime('%Y-%m-%d %H:%M:%S')}", normal_style))
        ...
        risk_color = {'CRITICAL': colors.HexColor('#e74c3c'), 'HIGH': colors.HexColor('#e67e22'), 'MEDIUM': colors.HexColor('#f39c12'), 'LOW': colors.HexColor('#17a2b8')}.get(risk_level, colors.grey)
        risk_style = ParagraphStyle('RiskStyle', parent=normal_style, textColor=risk_color, fontSize=14, fontName='Helvetica-Bold')
        ...
        if summary['sensitive_columns'] > 0:
            ...
            if len(findings_data) > 1:
                findings_table = Table(findings_data, colWidths=[2*inch, 2*inch, 2*inch, 1.5*inch])
            ...
            if report.get('recommendations'):
                ...
                for rec in report['recommendations']:
                    elements.append(Paragraph(f"<b>{rec['title']}</b> (Priority: {rec['priority']})", normal_style))
                    elements.append(Paragraph(rec['description'], normal_style))
                    elements.append(Spacer(1, 0.2*inch))
        else:
            elements.append(Paragraph("No Sensitive Personal Data Detected", heading_style))
            elements.append(Paragraph("This database appears to contain no GDPR-sensitive personal information based on column name analysis.", normal_style))
            elements.append(Paragraph("Note: This scan only analyzes column names. Manual review is still recommended to ensure compliance.", normal_style))
        ...
        buffer.seek(0)
        response = HttpResponse(buffer.read(), content_type='application/pdf')
        response['Content-Disposition'] = f'attachment; filename="{db_name}_gdpr_compliance_report.pdf"'
...
def gdpr_export_json(request):
    ...
    try:
        ...
        workspace = get_current_workspace(request)
        if not workspace or not workspace.sql_db:
            return JsonResponse({'error': 'No workspace or database selected'}, status=400)
        
        sql_db = workspace.sql_db
        
        # Check if GDPR report exists
        if not sql_db.gdpr_report:
            return JsonResponse({'error': f'GDPR compliance scan has not been performed yet for database "{sql_db.name}".'}, status=404)
        
        # Prepare response
        response = JsonResponse(sql_db.gdpr_report, json_dumps_params={'indent': 2})
        response['Content-Disposition'] = f'attachment; filename="{sql_db.name}_gdpr_compliance_report.json"'
...
def erd_export_pdf(request):
    ...
    try:
        ...
        # Check if ERD exists
        if not sql_db.erd or not sql_db.erd.strip():
            messages.error(request, f"ERD diagram has not been generated yet for database '{db_name}'. Please generate database documentation first.")
            return redirect('thoth_ai_backend:erd')
