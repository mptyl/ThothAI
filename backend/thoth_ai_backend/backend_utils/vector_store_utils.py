...
from thoth_qdrant import EvidenceDocument, ColumnNameDocument, SqlDocument, ThothType
...
def get_csv_export_path(request):
    ...
    try:
        ...
        sanitized_vect_type = "".join(
            c if c.isalnum() or c in ["_", "-"] else "_" for c in vect_type
        ).strip("_")
        if not sanitized_vect_type:
            sanitized_vect_type = "Unknown"
            logger.warning(
                "Vector database type resulted in empty string after sanitization, using 'Unknown'"
            )

        # Sanitize the vector database name for directory creation
        sanitized_vdb_name = "".join(
            c if c.isalnum() or c in ["_", "-"] else "_" for c in vdb_name
        ).strip("_")
        if not sanitized_vdb_name:
            sanitized_vdb_name = "default"
            logger.warning(
                "Vector database name resulted in empty string after sanitization, using 'default'"
            )
...
def get_vector_store(request):
    ...
    try:
        ...
        if not backend:
            raise ValueError(
                f"Vector database type {vector_db.vect_type} is not supported"
            )
        ...
        if embedding_provider == "openai":
            embedding_api_key = os.environ.get("OPENAI_API_KEY") or os.environ.get(
                "OPENAI_KEY"
            )
        elif embedding_provider == "cohere":
            embedding_api_key = os.environ.get("COHERE_API_KEY") or os.environ.get(
                "COHERE_KEY"
            )
        elif embedding_provider == "mistral":
            embedding_api_key = os.environ.get("MISTRAL_API_KEY") or os.environ.get(
                "MISTRAL_KEY"
            )
            ...
            embedding_api_key = os.environ.get("ANTHROPIC_API_KEY") or os.environ.get(
                "CLAUDE_API_KEY"
            )
        ...
        vector_store = VectorStoreFactory.create(backend=backend, **params)

        # Log embedding configuration source for debugging
        config_source = "environment variables"

        logger.info(
            f"Successfully created {backend} vector store for collection {vector_db.name} "
    ...
    try:
        ...
        if request is not None:
            try:
                ...
                logger.warning(
                    f"Failed to get new CSV export path: {e}. Falling back to old method."
                )
        ...
        if request is None:
            ...
            vector_db_type_folder = "".join(
                c if c.isalnum() else "_" for c in vector_db_type_folder
            ).strip("_")
        ...
        for doc in evidence_documents:
            ...
            ):  # Ensure it's the correct document type
                writer.writerow([getattr(doc, "id", ""), getattr(doc, "evidence", "")])
            else:
                logger.warning(
                    f"Skipping document of type {type(doc)} during evidence export, expected EvidenceDocument."
                )
...
def delete_all_evidence_from_vector_store(vector_store):
    ...
    try:
        ...
        if not hasattr(vector_store, "delete_collection"):
            logger.error(
                "Vector store object does not have a 'delete_collection' method."
            )
            raise NotImplementedError(
                "Vector store 'delete_collection' method not found."
            )

        vector_store.delete_collection(thoth_type=ThothType.EVIDENCE)
        logger.info(
            "Successfully called delete_collection for evidence (ThothType.EVIDENCE)."
        )
    except Exception as e:
        logger.error(
            f"Error deleting all evidence using delete_collection: {e}", exc_info=True
        )
        raise
...
def delete_all_columns_from_vector_store(vector_store):
    ...
    try:
        ...
        )  # Assuming ThothType.COLUMN_NAME
        logger.info(
            "Successfully called delete_collection for columns (ThothType.COLUMN_NAME)."
        )
    except Exception as e:
        logger.error(
            f"Error deleting all columns using delete_collection: {e}", exc_info=True
        )
        raise
...
def delete_all_questions_from_vector_store(vector_store):
    ...
    try:
        ...
        vector_store.delete_collection(
            thoth_type=ThothType.SQL
        )  # Assuming ThothType.SQL
        logger.info(
            "Successfully called delete_collection for questions (ThothType.SQL)."
        )
    except Exception as e:
        logger.error(
            f"Error deleting all questions using delete_collection: {e}", exc_info=True
        )
        raise
...
def import_evidence_from_csv_file(vector_store, request=None):
    ...
    try:
        with open(filepath, "r", newline="", encoding="utf-8") as f:
            ...
            for row in reader:
                try:
                    ...
                    if (
                        not evidence_id or evidence_text is None
                    ):  # evidence_text can be empty string
                        logger.warning(
                            f"Skipping evidence row due to missing id or evidence key: {row}"
                        )
                        error_count += 1
                        messages.append(
                            f"Skipped evidence row: missing id or evidence data - ID: {evidence_id if evidence_id else 'N/A'}"
                        )
                        continue

                    evidence_doc = EvidenceDocument(
                        id=evidence_id, evidence=evidence_text
                    )
                    vector_store.add_evidence(evidence_doc)
                    imported_count += 1
                except Exception as e_row:
                    logger.error(
                        f"Error importing evidence row (ID: {evidence_id}): {e_row}"
                    )
                    error_count += 1
        ...
        logger.info(
            f"Evidence import complete. Imported: {imported_count}, Errors: {error_count} from {filepath}"
        )
...
def import_columns_from_csv_file(vector_store, request=None):
    ...
    try:
            ...
            for row in reader:
                try:
                    ...
                    if not col_id:
                        ...
                        error_count += 1
                        messages.append(
                            f"Skipped column row: missing id - Name: {row.get('original_column_name', 'N/A')}"
                        )
                        continue
                    ...
                    csv_original_col_name = (
                        original_column_name_val
                        if original_column_name_val is not None
                        else ""
                    )
                    csv_value_desc = (
                        value_description_val
                        if value_description_val is not None
                        else ""
                    )
                    ...
                    column_doc = ColumnNameDocument(
                        ...
                        column_description=column_description_val
                        if column_description_val
                        else "",
        ...
        logger.info(
            f"Column import complete. Imported: {imported_count}, Errors: {error_count} from {filepath}"
        )
    except Exception as e_file:
...
def import_questions_from_csv_file(vector_store, request=None):
    ...
    try:
        with open(filepath, "r", newline="", encoding="utf-8") as f:
            ...
            for row in reader:
                try:
                    ...
                    q_sql = row.get("sql")
                    if (
                        not q_id or q_text is None or q_sql is None
                    ):  # question and sql can be empty strings
                        logger.warning(
                            f"Skipping question row due to missing id, question key, or sql key: {row}"
                        )
                        error_count += 1
                        messages.append(
                            f"Skipped question row: missing id, question or sql data - ID: {q_id if q_id else 'N/A'}"
                        )
                        continue
        ...
        logger.info(
            f"Questions import complete. Imported: {imported_count}, Errors: {error_count} from {filepath}"
        )
    except Exception as e_file:
    ...
    try:
        ...
        for doc in column_documents:
            if isinstance(doc, ColumnNameDocument):
                ...
                logger.warning(
                    f"Skipping document of type {type(doc)} during column export, expected ColumnNameDocument."
                )
    ...
    try:
        ...
        for doc in sql_documents:
            if isinstance(doc, SqlDocument):
                ...
                logger.warning(
                    f"Skipping document of type {type(doc)} during questions export, expected SqlDocument."
                )
