...
def convert_erd_to_vertical_flowchart(mermaid_content: str) -> str:
    ...
    for line in lines:
        ...
        if any(rel in line for rel in ['||--', '--||', '||..', '..||', '}|--', '--{|', 'o|--', '--o|', '}o--', '--o{']):
            ...
            if not any(c in line for c in [':', '{', '}']):
                ...
                if len(col_parts) >= 2:
                    col_name = col_parts[1] if col_parts[0] in ['PK', 'FK'] else col_parts[0]
                    col_type = col_parts[2] if col_parts[0] in ['PK', 'FK'] else col_parts[1] if len(col_parts) > 1 else ''
                    tables[current_table].append((col_name, col_type, 'PK' if col_parts[0] == 'PK' else 'FK' if col_parts[0] == 'FK' else ''))
    ...
    if not root_tables:
        ...
        root_tables = sorted(tables.keys(), key=lambda t: len(table_connections[t]['children']), reverse=True)[:2]
    ...
    for level_idx, level_tables in enumerate(all_levels):
        if level_tables:
            ...
            flowchart_lines.append(f'        direction LR')  # Within level, arrange left-to-right
            ...
            for table_name in level_tables:
                ...
                if columns:
                    ...
                    col_list = []
                    for col_name, col_type, col_attr in columns[:3]:  # Show first 3 columns for compactness
                        prefix = "🔑" if col_attr == 'PK' else "🔗" if col_attr == 'FK' else ""
    ...
    for rel_line in relationships:
        ...
        if len(parts) >= 3:
            ...
            if '||--' in rel_line or '--||' in rel_line:
                ...
                flowchart_lines.append(f'    {table1} --> {table2}')  # Zero-or-one-to-many
            else:
                flowchart_lines.append(f'    {table1} -.-> {table2}')  # Other relationships
...
def generate_mermaid_image(mermaid_content: str, output_format: str = 'svg', 
                          width: Optional[int] = None, height: Optional[int] = None) -> Tuple[bool, Optional[str], Optional[str]]:
    """
    ...
    try:
        ...
        encoded_diagram = base64.b64encode(mermaid_content.encode('utf-8')).decode('ascii')
        ...
        logger.info(f"Successfully generated Mermaid {file_extension.upper()}: {output_path}")
...
def generate_erd_pdf(mermaid_content: str, db_name: str) -> Tuple[bool, Optional[HttpResponse], Optional[str]]:
    """
...
def get_erd_display_image(mermaid_content: str) -> Tuple[bool, Optional[str], Optional[str]]:
    """
