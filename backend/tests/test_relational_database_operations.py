...
class RelationalDatabaseTestSuite(TestCase):
    ...
    def setUp(self):
        ...
        self.sql_dbs["sqlite"] = SqlDb.objects.create(
            name="test_sqlite", db_type=SQLDBChoices.SQLITE, db_name="test.db"
        )
    ...
    def test_data_type_mapping(self):
        ...
        self.assertEqual(
            map_data_type("CHARACTER VARYING(100)"), ColumnDataTypes.VARCHAR
        )
    ...
    def test_column_extraction_admin_action(self, mock_get_db_manager):
        ...
        test_table = SqlTable.objects.create(
            name="users", description="Test users table", sql_db=sqldb
        )
    ...
    def test_relationship_detection_admin_action(self, mock_get_db_manager):
        ...
        users_table = SqlTable.objects.create(
            name="users", description="Users table", sql_db=sqldb
        )

        orders_table = SqlTable.objects.create(
            name="orders", description="Orders table", sql_db=sqldb
        )
        ...
        order_user_id_column.fk_field = (
            f"{users_table.name}.{user_id_column.original_column_name}"
        )
    ...
    def test_relationship_pk_fk_fields_update(self):
        ...
        # Users table
        users_table = SqlTable.objects.create(name="users", sql_db=sqldb)
        user_id_col = SqlColumn.objects.create(
            original_column_name="id", column_name="id", sql_table=users_table
        )

        # Orders table
        orders_table = SqlTable.objects.create(name="orders", sql_db=sqldb)
        ...
        # Products table
        products_table = SqlTable.objects.create(name="products", sql_db=sqldb)
        product_id_col = SqlColumn.objects.create(
            original_column_name="id", column_name="id", sql_table=products_table
        )

        # Order items table with multiple FKs
        order_items_table = SqlTable.objects.create(name="order_items", sql_db=sqldb)
    ...
    def test_schema_operations_across_backends(self, mock_get_db_manager):
        ...
        for backend in backends_to_test:
            with self.subTest(backend=backend):
                ...
                sqldb = (
                    self.sql_dbs[backend]
                    if backend in self.sql_dbs
                    else self.sql_dbs["sqlite"]
                )
    ...
    def test_bulk_operations(self):
        ...
        columns_data = [
            ...
            {
                "name": "created_at",
                "type": ColumnDataTypes.TIMESTAMP,
                "desc": "Creation time",
            },
        ]
