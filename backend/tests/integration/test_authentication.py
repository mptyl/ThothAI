...
class TestAuthentication(TestCase):
    ...
    def test_session_authentication(self):
        ...
        try:
            ...
            if response.status_code != 302:  # Should redirect to login
                self._record_warning(test_name, {
                    'message': 'Protected page accessible without login',
                    'status_code': response.status_code
                })
            
            # Test login
            login_success = self.client.login(username='authuser', password='testpass123')
            if not login_success:
                self._record_error(test_name, {
    ...
    def test_token_authentication(self):
        ...
        try:
            ...
            self.api_client.credentials(HTTP_AUTHORIZATION=f'Token {self.user_token.key}')
            response = self.api_client.get('/api/workspaces')
            if response.status_code != 200:
                self._record_error(test_name, {
    ...
    def test_permission_isolation(self):
        ...
        try:
            ...
            response = self.client.post(reverse('set_workspace'), {
                'workspace_id': self.admin_workspace.id
            })
...
def generate_auth_summary():
    ...
    if os.path.exists(report_dir):
        for filename in os.listdir(report_dir):
            if filename.endswith('.json') and filename.startswith('test_authentication_'):
