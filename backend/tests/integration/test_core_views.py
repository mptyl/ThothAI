...
class TestCoreViews(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="coreuser", password="testpass123", email="core@example.com"
        )
    ...
    def test_api_login_view(self):
        ...
        try:
            ...
            response = self.api_client.post(
                reverse("api_login"),
                {"username": "coreuser", "password": "testpass123"},
            )
            ...
            response = self.api_client.post(
                reverse("api_login"), {"username": "coreuser", "password": "wrongpass"}
            )
    ...
    def test_set_workspace_session_view(self):
        ...
        try:
            ...
            response = self.client.post(
                reverse("set_workspace"), {"workspace_id": self.workspace.id}
            )
            if response.status_code != 302:  # Should redirect to login
                self._record_warning(
                    view_name,
            ...
            response = self.client.post(
                reverse("set_workspace"), {"workspace_id": 99999}
            )
    ...
    def test_get_tables_by_database_ajax(self):
        ...
        try:
            ...
            response = self.client.get(
                reverse("get_tables_by_database"), {"database_id": self.sql_db.id}
            )
...
def generate_summary_report():
    ...
    summary_file = (
        f"{report_dir}/summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
