...
class VectorDatabaseTestSuite(TestCase):
    ...
    def setUp(self):
        ...
        self.basic_model = BasicAiModel.objects.create(
            name="Test GPT Model", description="Test model for vector operations"
        )
    ...
    def test_vector_store_creation_chroma(self, mock_create):
        ...
        mock_create.assert_called_once_with(
            "chroma", collection="test_chroma_collection", path="/tmp/chroma_test"
        )
        self.assertEqual(vector_store, mock_store)
    ...
    def test_crud_operations_evidence_documents(self):
        ...
        mock_store.get_document.return_value = EvidenceDocument(
            id="evidence-id-1", evidence="Test evidence for database operations"
        )
        ...
        self.assertEqual(
            retrieved_doc.evidence, "Test evidence for database operations"
        )
    ...
    def test_evidence_preprocessing_integration(self, mock_get_vector_store):
        ...
        # Mock vector store
        mock_store = Mock()
        mock_store.bulk_add_documents.return_value = [
            "evidence-1",
            "evidence-2",
            "evidence-3",
        ]
        ...
        test_evidence = [
            EvidenceDocument(
                evidence="Primary key constraints ensure unique identification"
            ),
    ...
    def test_search_operations(self):
        ...
        evidence_results = mock_store.search_similar(
            "database constraints", ThothType.EVIDENCE
        )
    ...
    def test_collection_info_retrieval(self):
        ...
        mock_info = {
            ...
            "total_documents": 150,
            "document_types": {"evidence": 50, "columns": 75, "sql": 25},
            "backend_type": "qdrant",
            "status": "healthy",
        }
