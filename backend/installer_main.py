...
docker_status = {"running": False, "phase": "idle", "message": "Ready"}

# Docker Compose status for sister directory
docker_sister_status = {"running": False, "phase": "idle", "message": "Ready"}
...
def load_config() -> dict:
    ...
    try:
        with open(CONFIG_FILE, "r", encoding="utf-8") as f:
            ...
            databases = [
                db for db in config.get("databases", []) if db in VALID_DATABASES
            ]
            vectordbs = [
                vdb for vdb in config.get("vectordbs", []) if vdb in VALID_VECTORDBS
            ]
...
def save_config(databases: List[str], vectordbs: List[str]) -> None:
    ...
    config = {"databases": databases, "vectordbs": vectordbs}
...
def update_pyproject_local(databases: List[str]) -> None:
    ...
    if extra_databases:
        ...
        if "mariadb" in extra_databases and "sqlserver" in extra_databases:
            optional_deps["all-databases"] = ["mariadb>=1.1.0", "pyodbc>=4.0.0"]
    ...
    with open(PYPROJECT_LOCAL_FILE, "w") as f:
        if local_config:
            ...
            print(
                f"[SUCCESS] Updated {PYPROJECT_LOCAL_FILE} with default configuration"
            )
...
def update_sister_pyproject(sister_path: str, databases: List[str]) -> bool:
    ...
    try:
        ...
        if "thoth-dbmanager" in content:
            # Determine which extra databases are selected (beyond default PostgreSQL and SQLite)
            extra_databases = [
                db for db in databases if db not in ["postgresql", "sqlite"]
            ]
            ...
            with open(pyproject_local_path, "w") as f:
                if local_config:
                    toml.dump(local_config, f)
                    print(
                        f"[SUCCESS] Created {pyproject_local_path} with database selections"
                    )
                else:
                    # Empty config for default (PostgreSQL + SQLite only)
                    f.write(
                        "# Using default database configuration (PostgreSQL + SQLite)\n"
                    )
                    print(
                        f"[SUCCESS] Created {pyproject_local_path} with default configuration"
                    )
...
def update_requirements_file_legacy(
    file_path: str, databases: List[str], vectordbs: List[str]
) -> tuple[bool, bool]:
...
def update_pyproject_files(databases: List[str]) -> List[str]:
    ...
    try:
        ...
        system_dep_databases = {
            "mariadb",
            "sqlserver",
        }  # Database extras that require additional drivers
            ...
            for db in selected_system_deps:
                if db == "mariadb":
                    if system_platform == "darwin":
                        ...
                        local_install_instructions[db] = (
                            "Install MariaDB Connector/C for your platform"
                        )
                    ...
                    if system_platform == "darwin":
                        ...
                        local_install_instructions[db] = (
                            "Install MySQL client libraries for your platform"
                        )

                elif db == "sqlserver":
                    local_install_instructions[db] = (
                        "Install ODBC drivers from https://docs.microsoft.com/en-us/sql/connect/odbc/"
                    )
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                print(
                    "This is expected for local development if system dependencies are missing."
                )
                print(
                    "Docker deployment will work correctly with all selected databases."
                )
            else:
                        ...
                        if db == "mariadb":
                            ...
                            for path in mariadb_config_paths:
                                if os.path.exists(path):
                                    ...
                                    try:
                                        shutil.copy2(
                                            PYPROJECT_MERGED_FILE, PYPROJECT_FILE
                                        )
            ...
            if failed_local_databases:
                ...
                print(
                    "The following databases require system dependencies for local development:"
                )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error updating pyproject.toml: {str(e)}"
        )
    ...
    if len(updated_files) > 1:
        print(
            f"Backend + {len(updated_files) - 1} frontend project(s) updated with database extras"
        )
...
def run_docker_compose():
    ...
    try:
        ...
        docker_status["message"] = (
            "Installing dependencies with merged configuration..."
        )
        ...
        try:
            ...
            shutil.copy2(
                os.path.join(thoth_be_path, PYPROJECT_MERGED_FILE),
                os.path.join(thoth_be_path, PYPROJECT_FILE),
            )
        ...
        docker_status["message"] = (
            "Pulling Docker images (this may take several minutes on first run)..."
        )
        ...
        try:
            ...
            if pull_process.returncode != 0:
                ...
                print(
                    f"Warning: docker compose pull failed with output:\n{pull_stdout}"
                )
                docker_status["message"] = (
                    "Pull had issues, continuing with build (will pull during build)..."
                )
        except Exception as e:
            print(f"Warning: Exception during pull: {e}")
            docker_status["message"] = (
                "Skipping separate pull, will pull during build..."
            )

        # Now build and start containers (with --pull flag to ensure images are pulled)
        docker_status["phase"] = "building"
        docker_status["message"] = (
            "Building and starting containers (pulling images if needed)..."
        )
...
def run_docker_compose_sister():
    ...
    try:
        ...
        if not sister_path:
            ...
            docker_sister_status["message"] = (
                "Sister directory with docker-compose.yml not found"
            )
            return
        ...
        if os.path.exists(sister_pyproject):
            ...
            if not do_merge(
                ...
                print("Warning: Failed to merge Frontend configuration files")
        ...
        docker_sister_status["message"] = (
            "Pulling Frontend Docker images (this may take several minutes)..."
        )
        ...
        try:
            ...
            if pull_process.returncode != 0:
                # Don't fail - just log and continue
                print(
                    f"Warning: docker compose pull failed for Frontend with output:\n{pull_stdout}"
                )
                docker_sister_status["message"] = (
                    "Pull had issues, continuing with build (will pull during build)..."
                )
        ...
        docker_sister_status["message"] = (
            f"Building and starting Frontend containers in {sister_path} (pulling images if needed)..."
        )
        ...
        if process.returncode == 0:
            ...
            docker_sister_status["message"] = (
                "Frontend Docker Compose completed successfully"
            )
        else:
            # Check if it's actually an error or just warnings
            # Docker compose often returns non-zero exit codes for warnings
            if stdout and (
                "Successfully built" in stdout or "Successfully tagged" in stdout
            ):
                docker_sister_status["running"] = False
                docker_sister_status["phase"] = "completed"
                docker_sister_status["message"] = (
                    "Frontend Docker Compose completed with warnings"
                )
            else:
                docker_sister_status["running"] = False
                docker_sister_status["phase"] = "error"
                ...
                if actual_error:
                    docker_sister_status["message"] = (
                        f"Sister Docker Compose error: {actual_error[:500]}"
                    )
                else:
                    # Show last meaningful lines of output
                    meaningful_lines = [
                        l for l in error_lines if l.strip() and not l.startswith("#")
                    ]
                    if meaningful_lines:
                        docker_sister_status["message"] = (
                            f"Sister Docker Compose error: {meaningful_lines[-1][:500]}"
                        )
                    else:
                        docker_sister_status["message"] = (
                            f"Sister Docker Compose error: {error_output[:500]}..."
                        )
...
async def update_config(config: DatabaseConfig):
    ...
    if invalid_databases:
        raise HTTPException(
            status_code=400, detail=f"Invalid databases: {', '.join(invalid_databases)}"
        )
    ...
    try:
        ...
        success_message = (
            "Configuration updated successfully. Docker will use the new settings."
        )
...
async def start_docker_deploy():
    ...
    return DockerStatusResponse(
        status="started", message="Docker Compose started", phase="starting"
    )
...
async def get_docker_status():
    ...
    return DockerStatusResponse(
        status=status, message=docker_status["message"], phase=docker_status["phase"]
    )
...
async def start_docker_deploy_sister():
    ...
    return DockerStatusResponse(
        status="started", message="Frontend Docker Compose started", phase="starting"
    )
