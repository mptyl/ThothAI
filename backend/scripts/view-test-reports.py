...
def analyze_reports(reports: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for report in reports:
        ...
        if report.get('timestamp'):
            if not analysis['latest_run'] or report['timestamp'] > analysis['latest_run']:
        ...
        for warning in report.get('warnings', []):
            ...
            analysis['warning_by_view'][view] = analysis['warning_by_view'].get(view, 0) + 1
        ...
        for test, result in report.get('summary', {}).items():
            ...
            analysis['test_summaries'][test][result] = analysis['test_summaries'][test].get(result, 0) + 1
...
def display_analysis(analysis: Dict[str, Any]):
    ...
    print(f"Latest Test Run: {format_timestamp(analysis['latest_run']) if analysis['latest_run'] else 'N/A'}")
    ...
    if analysis['test_summaries']:
        ...
        for test, results in sorted(analysis['test_summaries'].items()):
            ...
            print(f"  {test}: {color}{pass_rate:.1f}% pass rate{Colors.NC} "
                  f"({results.get('PASS', 0)} passed, {results.get('FAIL', 0)} failed)")
    
    # Error distribution
    if analysis['error_by_view']:
        print_section("Views with Errors")
        for view, count in sorted(analysis['error_by_view'].items(), key=lambda x: x[1], reverse=True):
            print(f"  {Colors.RED}●{Colors.NC} {view}: {count} error{'s' if count > 1 else ''}")
    
    # Warning distribution
    if analysis['warning_by_view']:
        print_section("Views with Warnings")
        for view, count in sorted(analysis['warning_by_view'].items(), key=lambda x: x[1], reverse=True):
            print(f"  {Colors.YELLOW}●{Colors.NC} {view}: {count} warning{'s' if count > 1 else ''}")
...
def main():
    ...
    parser.add_argument('--latest', '-l', action='store_true', help='Show only the latest report')
    parser.add_argument('--report-dir', '-d', default='tests/reports', help='Report directory path')
