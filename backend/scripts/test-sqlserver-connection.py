...
def test_sql_server_connection(connection_string):
    ...
    try:
        ...
        if "ODBC Driver" in str(e):
            print(
                "  Suggestion: Try using 'ODBC Driver 18 for SQL Server' in the connection string"
            )
            print(
                "  Example: mssql+pyodbc://user:pass@server/db?driver=ODBC+Driver+18+for+SQL+Server&TrustServerCertificate=yes"
            )
            ...
            print(
                "  Suggestion: Add '&TrustServerCertificate=yes' to the connection string for self-signed certificates"
            )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("\nExamples:")
        print(
            "  pyodbc:  mssql+pyodbc://user:pass@server/db?driver=ODBC+Driver+18+for+SQL+Server"
        )
