...
- Provider AI in `ai_providers`. Almeno un provider deve essere abilitato e con API key. Ti suggerisco ovviamente OpenRouter perché tutta la configurazione del workspace demo è basata su OpenRouter.
- Impostazioni embedding (`embedding.provider`, `embedding.model`, e relativa API key o fallback a quella del provider). Deve esserci almeno un provider embedding abilitato. Ti suggerisco di iniziare con OpenAI ed usare la mia apikey fino a quando non avrai avuto tempo di provare Cohere. L'uso di Cohere però richiede di ricostruire la collection su Qdrant, per cui preferisco prima stabilizzare la situazione.
- Configurazione database (`databases`, con SQLite sempre abilitato). Il config.yml base ha già tutti i database che ti servono
- Sezione `admin` (almeno `username`, l'email è facoltativa). Inserisci la password admin123 o altra a tua scelta. 
- Inoltre devi inserire l'utente `demo` con password=demo1234 o altra password a tua scelta
- Sezione `monitoring` (se abilitata, richiede il `logfire_token`)
...
## 3) Esecuzione di `install.sh` oppure `install.ps1` per Windows su Powershell o `install.bat` per Windows su CMD:

Il flusso principale è implementato in `scripts/installer.py` ed è richiamato da `install.sh` o equivalenti per Windows. 

### Nota importante per Windows:
Prima dell'installazione vera e propria, gli script Windows (`install.ps1` e `install.bat`) eseguono una **conversione automatica dei line endings da CRLF a LF** per garantire la compatibilità con i container Docker Linux. Questa operazione:
- Converte automaticamente tutti i file di codice (`.sh`, `.py`, `.js`, `.ts`, `.jsx`, `.tsx`, `.yml`, `.yaml`, `.json`, `.toml`, `.ini`, `.conf`)
- Verifica in particolare i file critici come gli entrypoint Docker e docker-compose.yml
- Esclude automaticamente le directory `node_modules`, `.venv` e `.git`
- Mostra un report dei file convertiti e segnala eventuali problemi

Le fasi chiave del processo di installazione:

1. **Conversione line endings (solo Windows)**: conversione automatica da CRLF a LF per compatibilità Docker (righe 170-249 di `install.ps1`).
2. Caricamento e validazione configurazione: `ThothInstaller.load_config()` + validazioni base.
3. Password admin: richiesta o lettura da config; viene salvato l'hash in `.admin_password.hash`.
4. Generazione dipendenze locali: crea `backend/pyproject.toml.local` con extra DB necessari e `frontend/sql_generator/pyproject.toml.local`.
5. Merge `pyproject.toml`: produce `pyproject.toml.merged` in `backend/` e `frontend/sql_generator/` unendo base + local.
6. Generazione `.env.docker`: scrive tutte le variabili necessarie (provider AI, embedding, monitoring, admin, port mapping, ecc.). File: `.env.docker`.
7. Rete Docker: crea la rete se assente (default: `thoth-network`).
8. Volumi Docker: crea i volumi persistenti (`thoth-secrets`, `thoth-backend-static`, `thoth-backend-media`, `thoth-frontend-cache`, `thoth-qdrant-data`, `thoth-shared-data`).
9. Segreti Django: genera in volume `thoth-secrets` due file se mancanti: `django_secret_key` e `django_api_key`.
10. Build & up dei servizi: `docker compose build` e poi `docker compose up -d` sul file compose (default: `docker-compose.yml`).
11. Attesa backend: tenta connessione fino alla disponibilità.
12. Setup iniziale (solo installazione "greenfield"): invoca comandi `manage.py` che generano scope, documentazione e scan GDPR se c'è almeno un provider AI abilitato.
