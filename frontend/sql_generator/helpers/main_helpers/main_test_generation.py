...
async def generate_test_units(state, agents_and_tools, functionality_level=None, timeout_seconds: int = 20):
    ...
    """ 
    # Start timing for test generation
    from datetime import datetime
    state.execution.test_generation_start_time = datetime.now()
    ...
    try:
        ...
        # Complete timing for test generation
        state.execution.test_generation_end_time = datetime.now()
        if state.execution.test_generation_start_time and state.execution.test_generation_end_time:
            duration = (state.execution.test_generation_end_time - state.execution.test_generation_start_time).total_seconds() * 1000
            state.execution.test_generation_duration_ms = duration
            logger.info(f"Test generation completed in {duration:.2f}ms")